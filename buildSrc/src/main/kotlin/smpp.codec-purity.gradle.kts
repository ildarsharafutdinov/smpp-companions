...
// org.jspecify annotations jar (AD-35 compile-time null-safety; declare it compileOnly so codec
// RUNTIME stays {io.netty}+JDK). Enforced at GROUP granularity (netty-common/-transport transitives
// pass; any org.springframework* / com.nimbusds* / io.micrometer* / smpp.companion.proxy leak fails).
...
val allowedGroups: Set<String> = setOf("io.netty", "org.jspecify") // org.jspecify: AD-35 nullness annotations

tasks.register("enforceDependencyAllowlist") {
    group = "verification"
    description = "CODEC-040: fail if the codec classpath contains a module outside {io.netty, org.jspecify}."
    ...
    doLast {
        ...
        check(offenders.isEmpty()) {
            "CODEC-040: codec classpath contains module(s) outside the {io.netty, org.jspecify} allowlist " +
                    "(only io.netty:* + org.jspecify + JDK stdlib are permitted; AD-7 / AD-27 / AD-35): " +
                    offenders.joinToString(", ") { "${it.group}:${it.module}:${it.version}" }
        }
        logger.lifecycle("CODEC-040 OK — codec classpath is {io.netty, org.jspecify} + JDK stdlib only.")
    }
}

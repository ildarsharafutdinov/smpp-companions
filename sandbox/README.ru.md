...
ссылками на руководства как на образцы использования, эта страница переориентирована («С чего
...
Эта песочница оборачивает прокси в **реальный, немодифицированный сторонний SMPP-стек** —
Kannel 1.5.0 с обеих сторон плюс Keycloak, адъюдирующий каждый bind, — чтобы отлаживать и
доказывать корректность прокси против стека, который написали не вы. Она дополняет
автоматизированные наборы тестов и ничего не заменяет.

| Я хочу… | Куда идти |
|---------|-----------|
| Запустить развёртывание по-простому (docker) | Три руководства по вариантам использования — по одному на вариант (роль + режим), в стиле быстрого старта: [`reverse.mode-b`](runbooks/reverse-mode-b.ru.md) · [`forward.mode-a` + `reverse.mode-a`](runbooks/forward-reverse-mode-a.ru.md) · [`forward.mode-c` + `reverse.mode-c`](runbooks/forward-reverse-mode-c.ru.md) |
| Отладить прокси (отладчик IDE, `jcmd`, JFR) | §5 — рецепт `java -jar` на хосте (поза отладки) |
| Понять сам стенд (состав, порты, подъём) | §2–§4 |
| Доказать корректность сквозь цепочку | §6 — сценарии с ожидаемым наблюдением на каждом шаге |
| Найти, где что журналируется | §7 — точки входа отладки |
...
Цепочка с прокси в середине (SMPP-клиент фронтового bearerbox соединяется с host-ingress прокси,
...
flowchart TD
    ...
    prom["PROMETHEUS — compose-сервис<br/>namespace хоста · UI 127.0.0.1:9095"]
    ...
    osmpp -.-> pg
    sbearer -.-> pg
    prom -.->|"scrape /metrics — 127.0.0.1:9090 (loopback, общий namespace)"| proxy
    prom -.->|"всегда прописана — 127.0.0.1:9091, DOWN до второго инстанса из §5.6"| proxy
```
...
Единственное compose-стороннее исключение из паттерна `host.docker.internal` — это Prometheus
стенда (Story 8.2): сервис работает в режиме `network_mode: host` — в том же сетевом namespace, в
котором слушают обе позы прокси по умолчанию (jar из §5.2 на хосте и контейнеры руководств
на `--network host` — одинаково), — поэтому он собирает метрики с loopback-only `/metrics` по
AD-19 на литерале `127.0.0.1:9090` (плюс всегда прописанная цель второго инстанса
`127.0.0.1:9091`, §5.6) без расширения бинда и без публикации портов, а собственный
веб-интерфейс отдаёт только на loopback `127.0.0.1:9095`. Это фикстура стенда, а не продукт —
не-цель "no metrics dashboard / telemetry backend" остаётся в силе (аддендум A7); §4 поднимает
его вместе со всем остальным, а §5.3 читает его показания. Паттерн сбора метрик предполагает
Linux-хост — настоящий host networking (семантика `network_mode: host` на рантаймах класса
Docker Desktop иная).
...
| `conf/smsc-opensmppbox.conf` | opensmppbox на 14567: `smpp-logins` из `smsc-users.txt`, `route-to-smsc = FAKE1`, pgsql-DLR (`smsc_smpp_dlr`). Этот бокс — реальный SMSC, с которым соединяется egress прокси. |
...
Egress прокси говорит на SMPP 3.4 и должен соединяться с SMPP-**сервером**. fakesmsc не может быть этим
...
| 13001 | `front-bearer-box` | Порт фронтового бокса (sqlbox соединяется с ним через хост) |
| 13002 | `front-sql-box` | Порт sqlbox (smsbox соединяется с ним через хост) |
...
| 9095 | `prometheus` | Веб-интерфейс Prometheus стенда (8.2), биндится самим сервисом только на loopback `127.0.0.1:9095` — `network_mode: host` делает это прямым биндом в namespace хоста, а не публикацией compose (9090/9091 принадлежат прокси; 9093 — конвенция Alertmanager) |
...
| `smsc-opensmpp-box` | Admin-страницы нет; слушает 14567 (порт, с которым будет соединяться egress прокси); `docker compose logs smsc-opensmpp-box` показывает `Connected to bearerbox at host.docker.internal port 14001`. |
...
| `prometheus` | Включён всегда — поднимается обычным `docker compose up`: без compose-profile, без дополнительной команды (8.2). Фикстура: без healthcheck и без `depends_on` (зависимостей нет; цели сбора метрик на холостом ходу ОЖИДАЕМО в состоянии DOWN). Готов, когда `curl --retry 30 --retry-connrefused http://127.0.0.1:9095/-/ready` отвечает `Prometheus Server is Ready.` — и отвечает веб-интерфейс `http://127.0.0.1:9095/graph` (только loopback, как и всё на этом порту). |
...
и проверяются своей функцией, по таблице выше. Фикстура `prometheus` (8.2) не имеет ни
healthcheck, ни `depends_on`: она стартует безусловно, а состояние DOWN её целей до запуска
прокси — честное состояние простоя, а не сбой; её собственная готовность — curl `/-/ready` в
блоке экспресс-проверки ниже.
...
bearerbox соединяется с `host.docker.internal:2775`, а прокси не входит в compose-файл (он работает
...
Prometheus стенда показывает то же честное состояние простоя со своей стороны: обе его всегда
прописанные цели job `smpp-proxy` (`127.0.0.1:9090` и `127.0.0.1:9091`) читаются DOWN —
connection refused — пока не работает ни один прокси, а все остальные сервисы не затронуты
(пункт 7 из §5.3). Цель 9090 становится UP при первом запуске прокси — по §5.2 на хосте или
первым инстансом варианта §5.6 (строка §3); цель 9091 на любом одноинстансном стенде остаётся
DOWN навсегда (принято — она существует для двухинстансных вариантов использования §5.6).
...
Почему именно эти параметры варианта использования (каждый отражает факт проводки стенда):
...
| `bind.port=2775` | Контракт проводки стенда (`conf/front-kannel.conf` соединяется с `host.docker.internal:2775`). Это же умолчание yml — указано для самодокументирования. |
| `smsc.host=127.0.0.1`, `smsc.port=14567` | Прокси соединяется с compose-опубликованным opensmppbox по loopback хоста — реальный SMPP 3.4-сервер стороны SMSC (§2.1). |
...
3. **Сопряжение, в пределах ~10 с** (интервал повторных попыток фронтового bearerbox):
   ...
   прокси, ROPC-успех против compose-Keycloak (секрет из §5.1 делает реальную работу), соединение с
...
4. **`/metrics`** (loopback, read-only): `relay_binds_unknown_total 1.0`. Заметка честности:
   ...
   REVERSE-варианте использования у счётчика accepted нет предрегистрированного ряда, потому что reverse-варианты не
   ...
   существует только на forward-вариантах с таблицами маршрутизации. После первого интервала
...
7. **Prometheus стенда** (сервис 8.2, включён всегда; UI на `http://127.0.0.1:9095`): Status →
   Targets показывает job `smpp-proxy` с двумя всегда прописанными целями —
   `http://127.0.0.1:9090/metrics` **UP** (эндпоинт из пункта 4 — теперь сбор идёт каждые 5 с, а
   не вручную) и `http://127.0.0.1:9091/metrics` **DOWN** с ошибкой
   `dial tcp 127.0.0.1:9091: connect: connection refused` — цель второго инстанса из
   двухинстансных руководств §5.6, которого эта одно-проксивая поза никогда не запускает; обе
   остаются прописанными намеренно, чтобы двухинстансной форме не требовалась правка
   конфигурации (принятый компромисс, решение владельца 2026-09-20). Во вкладке Graph сопряжение
   уже записано в TSDB: `relay_binds_adjudication_seconds_count` = 1 (адъюдикация самого bind) и
   `relay_pdus_transit_seconds_count{direction="INGRESS"|"EGRESS"}` начинает тикать с первыми
   keepalive (§6.3) и накапливается сквозь сценарии §6 — каждый relayed PDU попадает в
   гистограмму транзита, счётчик каждого направления равен его `relay_pdus_total`. Сбои сбора
   метрик проявляются как чтение цели DOWN (`up == 0`) — ошибка на странице Targets называет
   класс: `connection refused`, когда никто не слушает, `context deadline exceeded`, когда сбор
   не укладывается в таймаут. Без запущенного прокси цель 9090 читается так же DOWN, при этом
   все остальные сервисы не затронуты — то самое честное состояние простоя из §4, увиденное
   отсюда, а не поломка.
...
для этого, а соединение с SMSC минует «шпильку» через хост; `-p 2775:2775` перепубликует ingress для
...
умолчанием остаётся запуск на хосте. Prometheus стенда тоже не видит этот вариант: `/metrics`
прокси на bridge-сети остаётся привязанным к loopback самого контейнера, недостижимым из
namespace хоста, в котором работает сервис `prometheus`, — задокументированное ограничение
варианта (его не разделяют руководства §5.6 на host network, где `/metrics` намеренно доступен
для сбора метрик); здесь это ничем не исправляется.
...
| `/metrics` | `relay_connections_closed_total{direction="INGRESS",reason="BIND_FAILED_NON_ROK"}` И `{direction="EGRESS",…}` +1 за повтор на ОБОИХ плечах; `relay_binds_rejected_total` не растёт; `relay_binds_unknown_total` не растёт | `relay_binds_rejected_total` +1 за повтор И `relay_binds_unknown_total` +1 (reverse-вариант использования считает каждый отказ в обоих); `relay_connections_closed_total{direction="INGRESS",reason="BIND_REJECTED"}` +1; НА EGRESS-плече активности нет |
...
| Веб-интерфейс Prometheus стенда | `http://127.0.0.1:9095` (строка готовности §4; только loopback) | точка наблюдения со стороны стенда: состояние целей job `smpp-proxy` (UP/DOWN у 9090/9091) и история счётчиков + гистограмм в TSDB — пункт 7 из §5.3 без ручного curl к `/metrics`. |
...
- **Bind не сопрягается** → сначала stdout прокси. Строки `bind_reject`: плечо OIDC (читайте
  ...
  `EGRESS_CONNECT_FAILED`: соединение egress — поднят ли opensmppbox, верен ли порт из §5.2?
...
| ВТОРОЙ контейнер составного варианта использования §5.6 отказывает при загрузке с отказом bind-in-use, называющим `metrics.port` | Оба контейнера прокси оставлены на умолчании 9090 — промах плана портов | Сдвиньте metrics у reverse: `--companion.metrics.port=9091` (запуск в руководстве уже так делает) |
| Составная цепочка §5.6 не сопрягается: stdout forward молчит, его `relay_connections_closed_total{INGRESS,EGRESS_CONNECT_FAILED}` растёт, на проводе фронта привычный 0x0d | Контейнер REVERSE лежит (или неверен его порт/сертификат) — egress-соединение forward и есть TLS-плечо | `docker ps` на контейнер reverse; `docker start` его (пересопряжение в пределах одного повтора — наблюдено) либо сверьте план портов руководства и SAN сертификата с `routing[0].host` |
...
| `prometheus` молчит в `docker compose ps`, а curl `/-/ready` из §4 не отвечает | 9095 занят на хосте (сервис умирает на бинде — healthcheck намеренно нет, поэтому `ps` молчит), либо `prometheus/prometheus.yml` перемещён (демон создаёт КАТАЛОГ по отсутствующему источнику монтирования — тот же класс `-v`-отказов, что в строке выше; на нём споткнулось и само T1-доказательство) | `docker compose logs prometheus` называет ошибку бинда; освободите 9095 (строка §3) или верните конфиг на место, затем `docker compose up -d prometheus` |
...
Именованный том `prometheus-tsdb` стоит на долговечной стороне той же формы: он ПЕРЕЖИВАЕТ
`docker compose down`, а удаляет его только `down -v` (дельта §10).
...
Контейнеры docker-вариантов использования §5.6 демонтируются командой `docker stop --timeout 30 <имя>` (exit 143
у каждого — java является PID 1 благодаря exec-form ENTRYPOINT; ожидание 30 с покрывает 10-секундный
дедлайн дрейна, по правилу останова deployment guide) и `docker rm <имя>`. В составных вариантах использования
...
7. **Руководства по docker-вариантам + `certs/` (T5, как записано 2026-09-18):** три руководства
   по вариантам использования в `runbooks/` (§5.6, EN + RU) — добавления только на стороне docker; compose-файл,
   conf Kannel и всё портированное не получают ничего (соединение `2775` фронтового conf обслуживает
...
8. **Сервис Prometheus + конфигурация сбора метрик (Story 8.2, как записано 2026-09-20):** девятый
   compose-сервис — `prom/prometheus:v3.14.0` (зафиксирован точно, прецедент Keycloak 26.7.0),
   `network_mode: host`, включён всегда (без compose-profile; стартует вместе с обычным
   `docker compose up`) — плюс `prometheus/prometheus.yml` (job `smpp-proxy`, две всегда
   прописанные цели на loopback `127.0.0.1:9090`/`127.0.0.1:9091`, интервал сбора 5 с) и
   именованный том `prometheus-tsdb` со сроком хранения 24 ч (стенд разработки, без обещаний
   долговечности — он ПЕРЕЖИВАЕТ `docker compose down`; удаляет его только `down -v`). Доступ к
   loopback-only `/metrics` по AD-19 достигается исключительно разделением namespace: без
   публикации `ports:`, без расширения бинда, без изменений под `proxy/` — паттерн сбора метрик

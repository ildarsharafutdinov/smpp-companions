...
flowchart TD
    ...
    prom -.->|"всегда прописана — DOWN до второго инстанса из §5.6"| proxy
```
...
| `prometheus` | Включён всегда — поднимается обычным `docker compose up`: без compose-profile, без дополнительной команды (8.2). Фикстура: без healthcheck и без `depends_on` (зависимостей нет; цели сбора метрик на холостом ходу ОЖИДАЕМО в состоянии DOWN). Готов, когда `curl http://127.0.0.1:9095/-/ready` отвечает `Prometheus Server is Ready.` — и отвечает веб-интерфейс `http://127.0.0.1:9095/graph` (только loopback, как и всё на этом порту). |
...
(пункт 7 из §5.3). Цель 9090 становится UP при первом запуске по §5.2; цель 9091 на любом
одноинстансном стенде остаётся DOWN навсегда (принято — она существует для двухинстансных
вариантов использования §5.6).
...
7. **Prometheus стенда** (сервис 8.2, включён всегда; UI на `http://127.0.0.1:9095`): Status →
   ...
   метрик проявляются как чтение цели DOWN (`up == 0`) с ошибкой connection refused на странице
   Targets: без запущенного прокси цель 9090 читается так
   же DOWN, при этом все остальные сервисы не затронуты — то самое честное состояние простоя из
   §4, увиденное отсюда, а не поломка.
...
8. **Сервис Prometheus + конфигурация сбора метрик (Story 8.2, как записано 2026-09-20):** девятый
   ...
   публикации `ports:`, без расширения бинда, без изменений под `proxy/`. Область фикстуры по
   аддендуму A7 — не-цель продукта "no metrics dashboard / telemetry backend" остаётся в силе;
   паттерн доступа доказан вживую до приземления сервиса (Implementation Notes Story 8.2).

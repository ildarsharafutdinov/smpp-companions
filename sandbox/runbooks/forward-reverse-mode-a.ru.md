...
| Отправка (сценарий README §6.1, без изменений) | HTTP 202; fakesmsc печатает текст байт-в-байт (в живом прогоне — `modeA-combo`); `relay_pdus_total{direction}` ОБОИХ инстансов симметрично +2/+2 на плечо для пары submit и ещё +2/+2 с возвратом DLR (вживую 4/4 на каждом); фронт считает `sent: sms 1 / rcvd: dlr 2` |

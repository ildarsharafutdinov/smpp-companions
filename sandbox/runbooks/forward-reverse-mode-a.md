...
| Metrics, one port per instance | Forward `curl -s http://127.0.0.1:9090/metrics`: `relay_binds_accepted_total{system_id="usr1"} 1.0` — the labeled routing-table series only a forward has. Reverse `curl -s http://127.0.0.1:9091/metrics`: `relay_binds_unknown_total 1.0`. |

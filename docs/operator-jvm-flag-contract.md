# Operator JVM-Flag Contract — the runnable JAR launch shape

> **Status:** ratified with Story 5.1 (2026-09-10; amended same day — this page is documentation
> only, nothing in the repository parses it) · **Audience:** operators launching the packaged proxy,
> and the later Docker entrypoint · **Owner decision:** bare `java -jar` + this page — no launcher
> script in the repository, and no launch configuration travels in the jar manifest.

This page is the **operator-facing definition** of the JVM flags a runnable-JAR launch must carry
...
through a story that touches both. The (later) Docker entrypoint must carry the same set — neither
...
single instance. The packaged shape is the smoke-tested shape, and it is smoke-tested on ZGC;
omitting the flag leaves the JVM-default collector — an untested GC posture, and (once the Docker
shape exists) a GC divergence between the two shapes, exactly what the one-flag-set rule exists to
prevent.
...
- **Packaged boot+smoke** (Story 5.1 T5): launches the built jar as a subprocess with exactly the
  set above, from a Java-side launch constant in the test source — the executable definition.
- **Docker entrypoint** (later Epic-5 story): must launch the same JAR with the same set; its story
  pins that parity.
- **This page**: the operator reference. By owner decision (2026-09-10) nothing in the repository
  parses it — no test, no launcher — so a flag change is a story that touches BOTH the constant and
  this page, and the reviewer checks they match.

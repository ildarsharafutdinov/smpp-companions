...
| Page | What it owns |
|---|---|
| [`deployment-guide.md`](deployment-guide.md) | taking an instance from zero to running: per-cell walkthroughs (forward A/C, reverse A/B/C) in both packaged shapes — the JAR launch and the canonical `docker run` recipe — plus the ROPC removal-track disclosure and the verbatim Mode A/B boot banners. Start here. |
...
the PERF-070/071 disclosure gates), the Kannel sandbox rig (Story 6.3 will own its README), and
packaged-shape conformance-run instructions (Story 6.2 owns the journey matrix). The SMPP 3.4

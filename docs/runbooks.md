...
| Metric | Kind | Labels | Meaning |
|---|---|---|---|
| `relay_binds_adjudication_seconds` | histogram | — (unlabeled by design — bucket series multiply per label value, and every candidate label is a forbidden dimension: the verdict type is log-only by the same rule as the reject counter, `system_id` would multiply × the routing table) | bind adjudication latency: verifier hand-off → verdict settle. ONE record per completed adjudication — every settled verifier future alike (`Allow`, both `Deny*`, an exceptional settle, and deadline/teardown-aborted exchanges: tearing down cancels the in-flight call, and that cancellation is itself a settle — the ROPC adapter's own guarantee, not the `VerdictRequest` port's (a future adapter must keep the settle promise for these exchanges to record) — so a deadline deny records at ~the deadline). Binds that never reached the verifier record nothing (routing miss, the bind-after-acceptor-stop gate, a verifier that failed synchronously before a future existed); after an `Allow` the record is spent at the settle, so a later failed egress dial adds no second record. Bucket edges `50ms · 100ms · 500ms · 1s · 2.5s · 5s · 8s · +Inf`, anchored to PERF-3 so each bind-latency anchor falls strictly between adjacent edges — the `companion.bind.adjudication-deadline` default (`4s`) sits inside (2.5 s, 5 s]. Role asymmetry as wired: the reverse cell's verifier is the real ROPC adapter while forward cells run the in-memory AlwaysAllow stand-in (`VerifierWiringConfig`) — so a forward cell's recorded latencies land in the lowest bucket (the stand-in settles in memory) and the meaningful latency signal is the REVERSE cell's (the same role asymmetry as `ropc_adjudications_active` below) |
| `relay_pdus_transit_seconds` | histogram | `direction` ∈ `INGRESS`\|`EGRESS` (both pre-registered — the closed set is the whole label domain) | per-PDU relay transit: framed-PDU arrival at the relay seam → forward onto the peer leg. One record per relayed post-couple PDU — the same single fire that counts `relay_pdus_total`; a PDU dropped to a dead peer records nothing (nothing was relayed). The peer's write is outside the measured window — this is the proxy's own forwarding work, not wire time. Bucket edges `100µs · 500µs · 1ms · 5ms · 10ms · 50ms · 100ms · 500ms · 1s · +Inf`, spanning sub-ms→s so the sub-ms PERF-4 per-PDU budget stays visible below the 1 ms edge (two edges under it) |

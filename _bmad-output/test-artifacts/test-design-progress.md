...
2. **Concurrency defects need a race-fuzz harness, not just load.** Single-shot conformance and the throughput run won't surface registry teardown races, half-close ordering, or write-failure propagation. ⇒ A **delay-injection / event-loop-jitter / randomized-channelInvalid soak** (the Epic 7 soak — re-pointed 2026-09-12 — is throughput-oriented, not race-oriented).

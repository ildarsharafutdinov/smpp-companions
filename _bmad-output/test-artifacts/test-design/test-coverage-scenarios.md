...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): the Docker half LANDED — the endpoint is in-container-only BY STRUCTURE.** The boot row's scrape arm runs `docker exec` of the image's OWN java against a helper class `docker cp`'d into the running container (distroless has no shell/curl): the literal-loopback listener on 127.0.0.1:9090 serves the full gauge surface to the in-container exec — `jvm_memory_used_bytes`, `jvm_gc_live_data_size_bytes` beside `relay_direct_memory_used_bytes` and the round itself (`relay_pdus_total{direction="INGRESS"} 1.0` / EGRESS). The structural half is pinned on the run itself: the container's port bindings contain ONLY the published SMPP listener — no 9090 mapping exists, so a `-p 9090` publish finds nothing listening; no host key exists to misconfigure the endpoint into exposure (AD-19/AD-28, the row's original drift concern — DEPLOY-011's Docker-arm home in the 5.2 spec). The JAR-side coverage is the packaged smoke's scrape of the same literal-bound endpoint (5.1, DEPLOY-015); the bind-class assertion family stays with OBS-004.

...
>
> **Row-status note (2026-09-12, Story 5.2 T5):** no rows added — the story lands DATED STATUS MARKERS ONLY (the catalog count stays **254**): DEPLOY-001/005/006/007/009 landed whole (001 owner-amended 2026-09-12: the crypto floor is defense-in-depth, not a runtime gate of the current path), DEPLOY-003/004/011/012's Docker halves landed (their JAR halves landed 5.1), DEPLOY-008 landed-as-amended (the structural inertness arm; owner decision 2026-09-11), DEPLOY-002/010 premise-voided (the relay is NIO by design — `RelayNettyConfig`; dated markers, no code change), DEPLOY-013's arm64 half owner-dropped ("no arm64 needed", 2026-09-11), SEC-075 owner-amended (structural stance — the all-Paths config surface is the mechanism, no guard), and SEC-077/098's Docker arms landed. Measured deploy actuals (recorded, not gated) live in the DEPLOY-001 marker: jlink runtime 97 MB, full image 144,197,650 bytes (~137.5 MiB), JVM-only container cold start ~0.6 s, full app container boot-to-startup-decision ~1.6 s.
>
> **Row-status note (2026-09-13, Story 6.1 T4):** no rows added — dated markers only (the count stays **254**): the docs-surface rows close OWNER-AMENDED — OBS-029..034, SEC-065, and SEC-049 (owner rule 2026-09-10: no Java test's oracle is a markdown page; the rows' docs-scan techniques are retired with the rule, the deliverable is the PAGE, and page↔reality coherence is the story's review sweep — the DEPLOY-003 "nothing in the repository parses the page" precedent, generalized). The satisfying pages are Story 6.1's: `docs/configuration.md` (T1), `docs/deployment-guide.md` (T2), `docs/runbooks.md` + `docs/cipher-allowlist-policy.md` (T3), indexed by `docs/README.md` (T4). OBS-035/036/037 carry refresh notes (the A-1 plan re-checked against as-built in place, falsifiable shape intact, the legacy `A1CarrierPlanDocsTest` GREEN through the refresh — legacy, not a pattern).
>
> **Row-status note (2026-09-16, Story 6.2 T4):** no rows added — dated markers only (the count stays **254**): **E2E-001 LANDED** as three suites (the in-JVM rung plus both packaged rungs) with the dated technique notes inside the row (the [B]-topology correction to the pre-amendment parenthetical, the real-verifier-over-stand-in and wire/log/metric-surface substitutions, both rungs, the SEC-098 cross-ref, and the mode A scope note of the ratified journey-widening decision, owner 2026-09-13); **DEPLOY-005's scope-honesty amended** (mode C composed + the in-session auth-DENY round now run in BOTH packaged shapes; mode A stays with the loopback suite + the structural-sameness argument).
>
> **Row-additions note (2026-09-17, Story 6.3 T4):** four E2E rows authored AS-LANDED for the Kannel manual rig — **E2E-002** (the happy send + DLR round trip through the real Kannel chain), **E2E-003** (`deliver_sm` MO injection — the A-1 session-affinity property against a real stack), **E2E-004** (`enquire_link` keepalive crossing the coupled pair), **E2E-005** (the two deny journeys — AD-32 case-4 verbatim vs AD-33 collapse) — all labeled **manual rig** (non-CI ops tier, the OBS-035/037 precedent: the documented journey with per-hop expected observations IS the proof artifact, executed live 2026-09-17 per `sandbox/README.md` §6). The catalog counts **258 scenarios** since (the §1.1/§1.2 tallies amended in step).
>
> **Row-status note (2026-09-18, Story 6.3 T5):** no rows added — a dated EXECUTION ADDENDUM for the 2026-09-17 rows' families (the count stays **258**): the owner-added T5 docker-combo runbooks (`sandbox/runbooks/` — reverse.mode-b, forward+reverse mode A, forward+reverse mode C, EN + RU) were each followed verbatim to `bind_accept coupled` on the live rig 2026-09-18 — E2E-002's send journey on the docker shape (byte-intact at fakesmsc through BOTH composed chains, per-PDU counters on each instance), E2E-004's keepalives crossing the pairs, a NEW mTLS REQUIRE-gate three-way probe (a plaintext PDU and a no-cert TLS dial to the mode C reverse never reach SMPP; the same probe with the client cert completes TLS 1.3), the composed reverse-down mutation on both pairs (E2E-005's failure surface on the FORWARD: silent stdout, `EGRESS_CONNECT_FAILED` climbing, the collapsed 0x0d at the front; restore re-couples), and the jar-swap restart cycle. Labeled **manual rig** (non-CI ops tier, the OBS-035/037 precedent; homes: `sandbox/runbooks/*.md`, indexed by `sandbox/README.md` §5.6). The §6.4 D1 deny arm is single-proxy by design — an off-table `system_id` is denied at the forward's routing table (the runbooks' routing-miss rows).
...
- **Status (Story 6.1 T4, 2026-09-13): OWNER-AMENDED — the doc half LANDED as the deliverable; the sampled-guard half stays retired with its REFRAMED caveat (2026-09-10 rule: no Java test's oracle is a markdown page, and a sampled profiler guard was never deterministic evidence).** `docs/runbooks.md` § "JFR and heap-dump hygiene" (Story 6.1 T3) is the guidance the row named: the two capture hazards stated against the in-code zeroize discipline (OldObjectSample interval sampling; heap dumps capturing unsettled adjudications, `OutOfMemoryError`-triggered dumps especially), the production guidance (JFR off unless actively diagnosing; prefer raising `-XX:OldObjectSampleInterval`; do not enable `-XX:+HeapDumpOnOutOfMemoryError` on a credential-path deployment — or treat the dump directory as secret-bearing), and the flag-contract rule that any JFR flag addition must ride BOTH deploy shapes or neither. The deterministic zeroization evidence remains SEC-046; nothing sampled is owed.
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — the doc-presence/content-scan technique OWNER-AMENDED (owner rule 2026-09-10: no Java test parses a docs page; deliverable = the page + review sweep).** The runbook exists: `docs/runbooks.md` § "Mode A ACL isolation — the two-proxy deployer controls" (Story 6.1 T3) documents the isolation requirement (one-way TLS cannot authenticate the connecting forward), the accepted residual (a submission oracle — the reverse still adjudicates, the SMSC stays the credential authority), and the operator controls the row names: listener ACL isolation via `companion.bind.host` + network ACLs, or Mode C `clientAuth(REQUIRE)`. Cross-linked to the verbatim `CompanionModeAWarning` banner (deployment guide) and the OBS-034 trust-anchoring controls in the same section.
...
- **Status (Story 5.2, 2026-09-12): OWNER-AMENDED — no config-level rejection of env-var VALUES exists or lands (owner decision 2026-09-11, "env is accepted"); the contract is upheld STRUCTURALLY.** Every `companion.*` secret key is a file `Path` (`oidc.client-secret-path`, trust-store path, cert/key paths — `application.yml`, commented examples only), so NO value key exists for an env var to bind: a secret VALUE in the environment is inert by construction, and SEC-076's all-Paths scan is the load-bearing mechanism that keeps it true (a value field cannot come to exist without failing that scan — the error-handling column of the original row, realized structurally). The Docker-shape E2E proving the stance end-to-end is `DockerSecretsE2eTest.envValueSecretIsInertTheMountedFileSecretIsTheOneHonored` (DEPLOY-008's marker): a decoy env VALUE under both relaxed-binding spellings is honored nowhere — the captured ROPC form carries the mounted FILE's secret — and appears in no stdout/log line.
...
- **Status (Story 5.2 T4, 2026-09-12, commit `cc0e764`): the Docker arm LANDED.** `DockerSecretsE2eTest.mountedSecretsReachReadyUnderTheNonRootUid` asserts the correctly-configured run's `docker inspect` Env carries NO secret material: no entry contains the mounted secret's VALUE, and no `COMPANION_*` key rides the env at all (paths ride ARGS; the mount is the only secret carrier — this daemon's base-image env is just `PATH` + `SSL_CERT_FILE`). The adversarial complement is row (b) (`envValueSecretIsInert…`): with a decoy VALUE deliberately injected, inspect Env provably CARRIES it (the leak channel DEP-1 forbids, premise pinned) while the app honors it nowhere and echoes it nowhere (no stdout/stderr line). The JAR-shape arm rides the same structure — the packaged smoke's child carries paths-only args and no secret VALUE exists to place in any environment (DEPLOY-015); the standing guarantee in both shapes is SEC-075's structural stance (no value key exists to bind).
...
- **Status (Story 5.2 T4, 2026-09-12, commit `cc0e764`): the image-layer arm LANDED (`DockerSecretsE2eTest.noSecretMaterialRidesAnyImageLayer`) — `docker save` unpacked, every layer blob scanned.** This daemon saves OCI-layout (`blobs/sha256/<digest>`, uncompressed despite the classic-looking `manifest.json`/`repositories` companions); the row's tar walk handles both layouts and gunzips gzip-magic blobs (some daemons compress). Marker choice is load-bearing (T4 finding): generic `-----BEGIN CERTIFICATE-----` markers FALSE-POSITIVE — the jlink runtime's own `lib/modules` carries the JDK's internal cert constants — and plaintext inside `proxy.jar` is invisible anyway (zip-deflated entries); the scan therefore matches the EXACT BYTES of every committed fixture cert/key/keystore (20 material files under `proxy/src/test/resources/keycloak/certs/`) plus the rigs' secret values — precisely the material a COPY would bake verbatim. Coverage is fenced by a tally (≥4 entries, >100 MB scanned — the runtime layer alone is ~101 MB) and detection by a verified must-hit control (mutation M3: a planted `JAVA_VERSION="` marker from the runtime's `release` file REDs the row naming the OCI blob). `docker history` pins exactly two payload layers (COPY jlink-image + COPY proxy.jar — buildkit and legacy created-by text both matched). Repo half, trued to as-built: the committed PKI lives ONLY under `src/test/resources/` (test fixtures, never `src/main` — no production secret rides the repo), and the image's two-COPY payload carries no cert/key/secret material by construction (the Dockerfile's SEC-098 build half, T2).
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (owner rule 2026-09-10: no Java test's oracle is a markdown page; the bidirectional consistency-scan technique is retired with the rule, the deliverable is the page, enforcement is the story's review sweep).** The page: `docs/configuration.md` (Story 6.1 T1) — every live `companion.*` key with type/default/fail-fast guard, sourced key-for-key from `application.yml` + the config records, with the RETIRED keys documented as retired alongside the `ignoreUnknownFields=false` refusal a stale config now gets (the R37 docs-mislead failure mode this row guards). T1's closing mutation was the manual bidirectional sweep this row originally wanted automated: every documented literal greps 1:1 against the oracles, and the reverse direction was done by record-component enumeration (every bindable key → a page row).
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (2026-09-10 rule; deliverable = the page + review sweep).** `docs/configuration.md` § "The role × mode matrix (AD-17)" (Story 6.1 T1) states all six cells — required/optional/structurally-absent per branch, forward×B forbidden (no `companion.forward.mode-b` node exists, SEC-051 structural), reverse×B opt-in-via-ack — sourced from the as-built property tree, not the spine's pre-implementation wording. The markdown-parser-vs-embedded-oracle technique is retired; the matrix's accuracy was swept against the records at T1 and rides every future config-story review.
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (2026-09-10 rule; deliverable = the page + review sweep).** `docs/deployment-guide.md` (Story 6.1 T2) carries all three: (a) the Mode B plaintext warning — the `CompanionModeBWarning` banner text quoted verbatim as the expected first stdout observation; (b) the opt-in ack — `companion.reverse.mode-b.acknowledged=true` documented with the SEC-052 refusal text a boot without it gets; (c) the reverse-only constraint — the configuration.md matrix's forbidden forward×B cell, cross-linked. The banner text was diffed line-for-line against the class source at T2.
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (2026-09-10 rule; deliverable = the page + review sweep); the Q3 blocker was resolved to AD-34 long since.** `docs/cipher-allowlist-policy.md` (Story 6.1 T3) publishes the policy: TLS 1.2 minimum / 1.3 preferred, both default cipher lists reproduced byte-for-byte from the shipped `application.yml`, the per-context empty-intersection fail-fast (three enforcement gates, refusal texts quoted), and the operator-tunable statement as a full tuning envelope. T3's closing mutation diffed the page's three default fenced blocks against the yml order-for-order — identical; the page/yml match is re-swept by every future `companion.tls.*` story's review.
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (2026-09-10 rule; deliverable = the page + review sweep).** `docs/deployment-guide.md`'s trust section (Story 6.1 T2) discloses the three-parter in substance: (a) v1 authenticates via ROPC / Direct Access Grants; (b) the RFC 9700 / OAuth 2.1 removal-track status; (c) the Keycloak build-pinning requirement — ≥26.7.0, re-validated each minor (the AD-12 accepted-risk register's recorded verdicts, not fresh research). It additionally carries the DAG enablement prerequisite the row's technique did not name: Direct Access Grants are per-client and OFF since Keycloak 26.2 — enable them or every bind denies fail-closed at first bind with the starred `OPERATOR_WARNING`.
...
- **Status (Story 6.1 T4, 2026-09-13): SATISFIED BY THE PAGE — technique OWNER-AMENDED (2026-09-10 rule; deliverable = the page + review sweep).** `docs/runbooks.md` (Story 6.1 T3) carries both halves: (a) the Mode A two-proxy ACL-isolation section — the "one-way TLS presents a cert; it never validates peers" warning (cross-linked to the verbatim `CompanionModeAWarning` banner in the deployment guide), the submission-oracle framing, and the two remediations (`companion.bind.host` scoping + network ACLs, or Mode C mTLS); (b) the egress trust-store never-cacerts default on every path, with the explicit public-PKI opt-in (importing the accepted roots into the dedicated store yourself — loud by scope) and the IdP rate-limit requirement.
...
- **Status (Story 6.1 T4, 2026-09-13): REFRESHED IN PLACE against as-built (not re-authored).** `docs/a-1-carrier-test-plan.md`'s PASS criterion survives verbatim (≥2 concurrent binds, same `system_id`, both `ESME_ROK`, on the real carrier); the refresh re-keyed the deployment preconditions to the as-built config (`companion.reverse.mode-b.smsc.host`/`.port`, the `acknowledged` opt-in, the [B]-topology plaintext SMSC leg) and cross-linked the launch walkthrough to `docs/deployment-guide.md`. The legacy `A1CarrierPlanDocsTest` gate stayed GREEN through the refresh — it remains legacy, not a pattern (2026-09-10 rule).
...
- **Status (Story 6.1 T4, 2026-09-13): REFRESHED IN PLACE against as-built (not re-authored).** The FAIL arms and the §5 procedure survive intact (2nd-bind rejection, DLR-on-the-wrong-bind, the submit-on-A/deliver-on-A-not-B assertions); the refresh updated the evidence half — the Epic-4 observability surface the 2.2-era plan said was future ("the SpliceObserver counters land with Epic 4") now exists and is named as-built (`bind_accept`/`bind_reject` log lines, the `/metrics` counters, cross-linked to `docs/runbooks.md`), and the chain description uses the unified couple/relay vocabulary. `A1CarrierPlanDocsTest` GREEN through the refresh.
...
- **Status (Story 6.1 T4, 2026-09-13): REFRESHED IN PLACE against as-built (not re-authored).** The oracle section is unchanged in substance — real target carrier (or an independent conformance SMSC) as system-under-oracle, the in-JVM mock EXPLICITLY excluded with the reason (it assumes A-1 and shares the codec's bugs); the refresh verified the cited mock path (`proxy/src/test/.../MockSmsc`) still resolves and dropped the plan's speculative `smpp-tools` client reference (no such tool exists in the repo — any SMPP 3.4 client qualifies). `A1CarrierPlanDocsTest` GREEN through the refresh.
...
- **Status (Story 5.2 T1, landed 2026-09-11, commits `08abada`+`5340110`; marker 2026-09-12 T5): LANDED — with the row's runtime-failure rationale OWNER-AMENDED 2026-09-12 (the floor is DEFENSE-IN-DEPTH, not a load-bearing runtime gate of the current path).** `:proxy:jlinkRuntimeImage` (`buildSrc/src/main/kotlin/smpp/deploy/JlinkRuntimeImageTask.kt`, cacheable, fail-closed on jdeps/jlink failure or an empty derived set) explodes the built boot jar's BOOT-INF layout and resolves the set by `jdeps --print-module-deps --ignore-missing-deps --recursive --multi-release 25` with the app classes as the sole root, adds the floor, and runs `jlink --strip-debug --no-man-pages` into `proxy/build/jlink-image/`; derivation evidence records at `proxy/build/jlink-derived-modules.txt` (12 jdeps-derived + 3 floor = 15 add-modules, 20 resolved). **The empirical finding behind the amendment** (three first-hand mutation arms — `jdk.crypto.ec` removed, `jdk.crypto.cryptoki` removed, both removed — each a full jlink → context → image → Docker-suite run): the RSA-fixture TLS handshake and the PKCS12 trust-store load complete on JDK 25 WITHOUT SunEC/cryptoki, so the row's predicted failure mechanisms are vacuous on the current PKI/JDK path; the floor's REAL bite is Gradle-input drift (a floor change re-runs the task, re-jlinks, and re-runs the suites against the rebuilt image — proven by the 2026-09-11 `jdk.crypto.ec` mutation) plus forward defense for EC certs / HSM profiles. **Measured actuals (recorded, not gated):** jlink runtime image 97 MB (`du`, `proxy/build/jlink-image/`); full Docker image 144,197,650 bytes (~137.5 MiB) = distroless base + COPY runtime 101 MB + COPY jar 21.9 MB (`docker history`, exactly two payload layers); container JVM-only cold start ~0.6 s (`docker run --entrypoint /opt/jre/bin/java … -version` under the four contract flags: 0.63/0.59/0.58 s); full app container boot-to-startup-decision ~1.6 s (`docker run` with no args → JVM + Spring context + the AD-17 zero-branch refusal, exit 1, pure JSON stdout). The spine's ~45–66 MB targeted the runtime and the epic-context "~50–100 MB / <500 ms" pair was never ratified — the figures above are the recorded actuals.
...
- **Status (Story 5.2, 2026-09-12): PREMISE-VOIDED (owner amendment 2026-09-11; dated marker, no code change — the DEPLOY-014 precedent).** The ratified row presumes an epoll transport the codebase never adopted: the relay is Netty-4.2 NIO BY DESIGN (`RelayNettyConfig` builds `MultiThreadIoEventLoopGroup(NioIoHandler.newFactory())`), no `netty-transport-native-epoll` classifier ships in the jar (an image scan would have nothing to find), and Epic 5 owns no main source package — adopting epoll to satisfy the row is out of contract. The "silent NIO fallback" drift vector does not exist here: NIO IS the transport (see DEPLOY-010's matching marker).
...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): the Docker half LANDED — the third kept-in-step representation exists and is pinned.** The exec-form ENTRYPOINT carries the set verbatim: `DockerImageBootSmokeTest.imageEntrypointCarriesTheContractFlagSetVerbatim` asserts `docker inspect` ENTRYPOINT == `[/opt/jre/bin/java]` + `PackagedBootSmokeTest.OPERATOR_JVM_FLAGS` + `[-jar, /opt/proxy.jar]` exactly — and, in the same row, `USER nonroot:nonroot` plus NO env flag-fork (the image env carries none of `JDK_JAVA_OPTIONS`/`JAVA_TOOL_OPTIONS`/`JAVA_OPTS`/`_JAVA_OPTIONS`). The preview-API arm rides the successful boot itself: the app IS preview-compiled, so the boot row's `startup_summary` under this ENTRYPOINT is the flag working (the introspection technique named by the row is realized as the JAR shape's live `/proc/<pid>/cmdline` pin, DEPLOY-015). Mutation (2026-09-12): ENTRYPOINT `-XX:+UseZGC` → `-XX:+UseG1GC` re-ran the suite against the REBUILT image (the everything-the-image-bakes input-trap proof) → this parity row RED.**
...
- **Status (Story 5.1, 2026-09-10): the JAR half LANDED in the same contract + smoke rows; the Docker half stays OPEN.** The packaged-smoke happy row asserts `-XX:+UseZGC` and `-XX:MaxDirectMemorySize=6442450944` verbatim on the `/proc/<pid>/cmdline` launch, and the MaxDirectMemorySize half is additionally BEHAVIORAL: the smoke passes no `companion.memory.*` args, so the child boots on the shipped yml trio and the `startup_summary` line pins `memory_budget_bytes == direct_memory_ceiling_bytes == 6,442,450,944` (the AD-30 derivation 65536 × 64 × 1024 × 1.5 — reaching ready at all IS the `budget-check: fail` pass, and retuning either side without the other turns the row RED, exactly the operator retune rule the contract page states). The GarbageCollectorMXBean 'ZGC'-name introspection arm of the original technique defers to the Docker-half parity row: collector identity rides the gc-labeled `jvm_gc_*` timer series, which `JvmGcMetrics` creates lazily on the first GC notification — nondeterministic timing within a smoke window (the `/metrics` scrape itself is reachable, the smoke scrapes it; determinism, not reachability, is the blocker), so the full runtime-introspection family returns there.**
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): the Docker half LANDED — including the introspection arm 5.1 deferred here.** The same parity row pins `-XX:+UseZGC` and `-XX:MaxDirectMemorySize=6442450944` verbatim on the ENTRYPOINT; the boot row pins the behavioral interlock identically to the JAR row (NO `companion.memory.*` args ride the container — `startup_summary` carries `memory_budget_bytes == direct_memory_ceiling_bytes ==` the constant, so retuning the yml trio or the flag without the other turns the row RED). The deferred GarbageCollectorMXBean arm landed as `DockerImageBootSmokeTest.forcedGcThenInContainerScrapeNamesZgc`: a FORCED GC cycle in the PID-1 proxy (jcmd `GC.run` spoken by hand over the JDK attach protocol on AF_UNIX — the jlink runtime ships no `jdk.attach` and the app runs no JMX agent) makes the lazily-created timer families deterministic, and the in-container scrape asserts `jvm_gc_pause_seconds` and `jvm_gc_concurrent_phase_time_seconds` named `gc="ZGC` on cause "Diagnostic Command" (JDK 25 generational ZGC names, empirically confirmed). The 5.1 blocker (lazy-family timing nondeterminism within a smoke window) is answered by forcing the cycle instead of awaiting it — determinism, not reachability.
...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): LANDED — the Docker twin of the DEPLOY-015 JAR spine, over the same jar bytes.** `DockerImageBootSmokeTest.dockerBootRelaysOneRoundAndDrainsOnDockerStop` mirrors `PackagedBootSmokeTest` row for row on the image that packages 5.1's `proxy.jar` byte-for-byte (FR-DEPLOY-1): identical stdout literals (`startup_summary` cell fields + the AD-30 interlock, the MODE B WARN banner — on stdout, absent from the container's stderr —, `bind_accept` naming `system_id`, the OBS-020 drain WARN), identical wire literals (ROK `bind_transceiver_resp` sequence-correlated, byte-exact `submit_sm`/`deliver_sm` both legs), the identical flag set (ENTRYPOINT ≡ `OPERATOR_JVM_FLAGS`, pinned by the parity row), and the identical Spring run-args channel (the CMD pass-through after the ENTRYPOINT = the `java … -jar proxy.jar <args>` surface). The deterministic allow verdict comes from the IMMEDIATE-allow token stand-in over a TLS-verified portal hop (`host.testcontainers.internal` + the SAN-matched `docker-host` fixture cert — re-pointed, never weakened). Scope honesty, both shapes: the smoke journey is the reverse-B cell + allow path; DENY/refusal outcomes in the Docker shape are pinned by T4's refusal rows (DEPLOY-009) and the AD-17 zero-branch refusal, and mode A/C journeys were never e2e-scoped in either shape — the parity claim rests on the smoke journeys plus the structural sameness (one jar, one arg channel, one flag set).
- **Status (Story 6.2 T3/T4, 2026-09-16): the scope-honesty sentence above is trued — mode C composed and the in-session auth-DENY round now run in BOTH packaged shapes.** `ComposedPackagedE2eTest` (two `java -jar` subprocesses under the same `OPERATOR_JVM_FLAGS`, secrets by file path — DEP-1's contract) and `ComposedDockerE2eTest` (two containers of the same image through the folded `testsupport/DockerRig`) drive the composed `forward.mode-c ↔ reverse.mode-c` chain: the jSMPP allow round (byte-pinned bind/submit bodies, the DLR on the originating pair), the 401-stand-in auth-DENY round (ONE `ESME_RBINDFAIL` on the wire, no couple, the rich verdict only on the reverse), and the composed REL-3 drain (SIGTERM/`docker stop` to each → the ordered stream → 143, both shapes). Mode A deliberately stays off the packaged shapes (owner decision 2026-09-13, the ratified journey-widening default): composed mode A is `TlsModesLoopbackE2eTest`'s at relay altitude, and A's parity claim remains the structural sameness — one jar, one arg channel, one flag set. The reverse-B smoke journeys and refusal rows above are unchanged; the operator-facing matrix is `docs/deployment-guide.md` § "What is machine-proven about these shapes".
...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): LANDED — the stop half rides the boot row, the exec-form half the parity row.** `DockerImageBootSmokeTest.dockerBootRelaysOneRoundAndDrainsOnDockerStop` drives `docker stop` (SIGTERM, 30s timeout) on a container holding a LIVE coupled pair: java is PID 1 via the exec-form ENTRYPOINT (`proxy/src/docker/Dockerfile`), the ordered stdout chain is pinned positionally — `startup_summary` < the `bind_accept` couple line < the OBS-020 drain WARN naming the configured PT2S deadline (the live pair force-closed at the deadline) — and the container exits **143** (the JVM hook-completed-SIGTERM convention; a shell-form `/bin/sh` PID 1 would strand the walk and time the stop out). The exec-form assertion itself is pinned by `imageEntrypointCarriesTheContractFlagSetVerbatim` (docker inspect ENTRYPOINT is a JSON array). The JAR-shape twin (SIGTERM → ordered drain → 143) is DEPLOY-015's smoke — the shape pair is closed.
...
- **Status (Story 5.2 T4, 2026-09-12, commit `cc0e764`): LANDED — the UID proof is LIVE, not image text.** `DockerSecretsE2eTest.mountedSecretsReachReadyUnderTheNonRootUid`: `docker inspect` State.Pid → the host kernel's `/proc/<pid>/status` pins `Uid:`/`Gid:` = 65532 (the distroless `nonroot`) on every field (real/effective/saved/fs) of the PID-1 java; both mounted `/run/secrets` files (client secret + trust store) stat `perms=r--r--r--` from an in-container exec running AS the container's user (`docker exec` runs the image UID, so the probe's `Files.isReadable` is the real access(2) of the non-root JVM); and the container reaches ready (`startup_summary`). The image text pins `USER nonroot:nonroot` explicitly (T2). Technique honesty (the spec's Design Note): an unprivileged host test cannot `chown` to 65532 — the world-readable mounted mode is the host-side half, and the LOAD-BEARING half (the non-root JVM reads the file) is proven by mutation M2: mounting the file 0600 root-owned flips the row into the DEPLOY-009 refusal — a root JVM would have read it and booted (verified RED, then GREEN on restore).
...
- **Status (Story 5.2 T4, 2026-09-12, commit `cc0e764`): LANDED AS AMENDED (owner decision 2026-09-11, "env is accepted") — the row's refusal language is replaced by the STRUCTURAL-INERTNESS arm.** No env-channel guard exists or lands: AD-18 is upheld structurally (every `companion.*` secret key is a file `Path`, no value key exists to bind — SEC-076's all-Paths scan is the load-bearing mechanism; see SEC-075's matching marker). `DockerSecretsE2eTest.envValueSecretIsInertTheMountedFileSecretIsTheOneHonored` injects a decoy secret VALUE into the container env under BOTH relaxed-binding spellings of the would-be value key (`COMPANION_REVERSE_MODEB_OIDC_CLIENTSECRET` — the uniform-form name Spring's binder WOULD resolve — and the naive `..._CLIENT_SECRET`), pins the premise real (`docker inspect` Env carries the decoy — the leak channel DEP-1 forbids), then proves inertness WIRE-LEVEL: the run boots to ready solely on the mounted file-path secrets, and the ROPC token form captured at the stand-in IdP carries `client_secret=` the MOUNTED FILE's value, never the decoy (a future value field that outranked the path, or an env fallback, turns the row RED — mutation M1 verified, live captured form printed); the decoy appears in no stdout/stderr line.
...
- **Status (Story 5.2 T4, 2026-09-12, commit `cc0e764`): LANDED — both bad states, refusal texts pinned from live runs.** `DockerSecretsE2eTest.missingAndUnreadableSecretFilesRefusePreBind`: (a) the configured `client-secret-path` pointing at a non-existent file and (b) the file mounted mode 0600 root-owned (unreadable by 65532 — docker cp preserves the host-chmod'd mode through Testcontainers' tar) each exit **1** at startup validation with the SEC-060/AD-18 failure line naming the path and the state — `...client-secret-path=/run/secrets/oidc-client-secret does not exist (OIDC client secret file missing)` / `is not readable (OIDC client secret file permissions)` — and NO `startup_summary` (the validation fired during the context refresh: no listener ever binds, no partial start). The in-container proof rides `CompanionConfigValidator.requireReadableFile`'s existing matrix unchanged — 5.2 added no main-source line (pure packaging + tests).
...
- **Status (Story 5.2, 2026-09-12): PREMISE-VOIDED (owner amendment 2026-09-11; dated marker, no code change — the DEPLOY-014 precedent, mirroring DEPLOY-002).** The ratified row presumes an epoll transport with an NIO *fallback*; the codebase never adopted one: `RelayNettyConfig` builds `MultiThreadIoEventLoopGroup(NioIoHandler.newFactory())` — NIO IS the deliberate transport (Netty 4.2), no `netty-transport-native-epoll` classifier ships in the jar, and the row's "silent fallback" defect class cannot exist (there is nothing to fall back FROM). Epic 5 owns no main source package, so adopting epoll to satisfy the row is out of contract. The Docker E2E relay rounds (T3/T4) run on — and prove — the NIO transport the image actually ships.
...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`; marker trued same day in the review round): the Docker half LANDED — the endpoint is in-container-only BY STRUCTURE.** The boot row's scrape arm runs `docker exec` of the image's OWN java against a helper class `docker cp`'d into the running container (distroless has no shell/curl): the literal-loopback listener on 127.0.0.1:9090 serves the full gauge surface to the in-container exec — `jvm_memory_used_bytes`, `jvm_gc_live_data_size_bytes` beside `relay_direct_memory_used_bytes` and the round itself (`relay_pdus_total{direction="INGRESS"} 1.0` / EGRESS). The structural half is pinned on the IMAGE text: `docker inspect` of the built image carries NO ExposedPorts declaration at all — an `EXPOSE 9090` regression in the Dockerfile REDs the row (the run's port bindings only mirror the rig's own publish config and could never bite on an image change); no host key exists to misconfigure the endpoint into exposure (AD-19/AD-28), so a `-p 9090` publish finds nothing listening. The JAR-side coverage is the packaged smoke's scrape of the same literal-bound endpoint (5.1, DEPLOY-015); the bind-class assertion family stays with OBS-004.
...
- **Status (Story 5.2 T3, 2026-09-12, commit `5340110`): the Docker half LANDED — behavioral, on the round's live egress leg.** The container's egress (the ROPC token dial AND the SMSC dial) crosses the Testcontainers host-access portal by name (`host.testcontainers.internal` — an IPv4 `/etc/hosts` entry added by `withAccessToHost`) under the ENTRYPOINT's `-Djava.net.preferIPv4Stack=true` (pinned verbatim by the parity row): the boot row's full bind→couple→relay round COMPLETING over that name is the Inet4-connect proof — both satellite dials connect over IPv4 with full TLS hostname verification (the SAN-matched `docker-host` fixture pair, generate.sh §11 — re-pointed, never weakened). The flag-presence half (both shapes) is DEPLOY-003/004's introspection family; the row's remote-address-class technique is realized as the completed verified round (an AAAA-first dial could not complete the hostname-verified TLS hop), and the JAR-side behavioral twin is the packaged smoke's loopback round under the same flag (DEPLOY-015).
...
- **Status (Story 5.2, 2026-09-12): the arm64 half OWNER-DROPPED (owner decision 2026-09-11: "no arm64 needed") — the row closes as owner-amended, the DEPLOY-014 precedent.** No dual-arch CI matrix lands and none is owed: no CI exists in this repo at all (the packaged-shape E2E rides `@Testcontainers(disabledWithoutDocker = true)` on the dev box — Docker 29.6.1, x86_64, cgroup v2, verified 2026-09-11), and release/publish tooling was explicitly out of the story's scope (spec Never-list, 2026-09-11). The row's original rationale — the ARM Epoll-classifier omission, DEPLOY-002's runtime backstop — died with DEPLOY-002/010's premise-voiding: the relay is NIO by design and no native classifier exists for EITHER arch. The jlink runtime, image, and toolchain are x86_64-only today (glibc, matching the distroless base); an arm64 port would be a new owner decision, not owed work.
...
### 8.1 Composed two-instance flow (E2E-001, Story 6.2)
...
- **Status (Story 6.2 T2/T3 landed 2026-09-16, commits `9cc0eb9` + `d18c71f`; marker at T4): LANDED — BOTH rungs, as three suites, with dated technique notes.** (1) *Topology:* the row's pre-amendment parenthetical ("forward → OIDC adjudication → … reverse", "a bind the forward already adjudicated") reads THROUGH the AD-12 amendment of 2026-08-18 — as-built [B], the REVERSE is the sole adjudicator and the forward is a trusted-side relay carrying no OIDC material that dials the reverse per SMPP session; the row text above stays frozen (the frozen-row precedent) and this note is the dated correction. (2) *Verifier substitution:* the "fake BindCredentialVerifier" tooling line is satisfied by the REAL `RopcBindCredentialVerifier` over the TLS token stand-in (`TokenIdpStandIn`: the immediate-200 allow arm yields a genuine Allow; the parked-401 arm yields DenyInvalid) — deterministic, no Keycloak boot cost; a live-`TrustOnlyKeycloakContainer` variant was an owner amendment not taken (the 3.2 live suite already proves the real adapter against real Keycloak; the composed gap was the CHAIN, not the IdP), and this run also closes the 3.3 close-out ROPC-through-TLS gap. (3) *Assertion-surface substitution:* "capturing SpliceObserver" (a harness-era assumption) is satisfied by the real application's OWN surfaces — the WIRE (the jSMPP 3.0.2 ESME drives the allow round: COMP-1's independent stack binds/submits/receives through its own strict parser, with the bind/submit bodies pinned byte-exact against jSMPP's deterministic serialization at the SMSC; the DENY/REQUIRE rows use the raw-socket client because their load-bearing observation is the EXACT on-wire collapse — a 16-octet header-only `ESME_RBINDFAIL` 0x0000000D then close — which jSMPP surfaces only as a generic `IOException`; jSMPP stays on the path via the allow round), the JSON-LOG events (`startup_summary` / `bind_accept` / `bind_reject` on each instance's own stdout, the production encoder's stream), and the per-instance `/metrics` scrapes — the DEPLOY-001 OWNER-AMENDED precedent. (4) *Both rungs:* the in-JVM rung (`bootstrap/ComposedChainE2eTest`, 3 rows — allow / auth-DENY / a full-boot Mode C REQUIRE-negative whose foreign-CA dial never reaches SMPP, `EGRESS_CONNECT_FAILED` on the forward's close taxonomy) is the row's "two in-JVM proxy instances" letter; the packaged rung (`bootstrap/ComposedPackagedE2eTest` — two `java -jar` subprocesses under the ONE operator flag set with file-path secrets, and `bootstrap/ComposedDockerE2eTest` — two containers of the one image through the folded `testsupport/DockerRig`) is the spine AD-24 note's "real Spring Boot processes … also validates DEP-1" letter. Journeys per rung: the ALLOW round (ROK observed BY the jSMPP ESME, the bind frame VERBATIM at the SMSC — AD-14 — `submit_sm` byte-intact through both hops, the DLR on the originating composed pair — A-1 across two proxies), the in-session auth-DENY round (ONE generic code, no couple anywhere — no SMSC session, no accept line — the rich verdict only in the reverse's `bind_reject` line + `relay_binds_rejected_total`; AD-33), and REL-3 on the packaged rungs (SIGTERM/`docker stop` to each instance with the pair open → the ordered `startup_summary < bind_accept < drain WARN` stream → exit **143**, with the forward-stopped-first asymmetry stated: the reverse's pair dies with the forward's drain, its walk short-circuits on an empty registry — still 143). (5) *SEC-098 cross-ref:* the FR-AUTH-3 per-instance-cert property held across two real instances in every rung — the forward's own client pair against the reverse's server cert + REQUIRE trust store, distinct material per instance, and the REQUIRE-negative row proves the anchors bite. (6) *Mode A scope note (the ratified journey-widening default, owner 2026-09-13):* the widening = mode C composed + in-session auth-DENY in both shapes; mode A stays with `TlsModesLoopbackE2eTest` (composed mode A at relay altitude) + the DEPLOY-005 structural-sameness argument — recorded, not silently ignored. Tier semantics per the ratified decision: the suites live in the DEFAULT test discovery (no Gradle gating); the "weekly/manual tier" is realized as the documented operator run — `docs/deployment-guide.md` § "The packaged-shape conformance run" — whose commands were executed from the page at T4. The three rungs' shared wire/log/scrape literals live in ONE home, `testsupport/ComposedJourney` (the BH7 discipline applied to the composed family). (7) *Owner ratification (2026-09-16, review round 1):* the REL-3 forward-first drain asymmetry is OWNER-RATIFIED — every coupled pair traverses both instances, so the second-stopped instance always drains an empty registry; the frozen "each (pair open)" letter is satisfied by the forward-first full-chain observation PLUS the reverse's empty-registry short-circuit (both exits 143, both shapes), not by a mirrored second stop arm.

### 8.2 The Kannel manual-rig journeys (Story 6.3, 2026-09-17 — non-CI ops tier)

The rig (`sandbox/`): docker-compose Kannel 1.5.0 on both sides — front smsbox HTTP / sqlbox / bearerbox SMPP-transceiver client; SMSC side opensmppbox (the real SMPP 3.4 server) / bearerbox / fakesmsc, pgsql DLR store — plus a compose Keycloak (26.7.0, fixture-mirrored realm, ROPC) adjudicating every bind, with the packaged `java -jar` proxy (reverse.mode-b, the ONE operator flag set) on the host wedged between: the front dials the proxy's host ingress 2775, the proxy dials the published opensmppbox 14567. COMP-1 evidence beyond the in-repo mocks and jSMPP — a REAL unmodified third-party SMPP 3.4 stack — that complements, never replaces, the automated oracles (journey automation is an explicit dispatch decision, not a default).

**E2E-002** `[manual rig — Story 6.3]` — The happy send: `submit_sm` relayed verbatim through the proxy + the DLR round trip (front smsbox HTTP → proxy → opensmppbox → fakesmsc → back)
Level: e2e · Priority: P2 · Risks: R33 · NFR: COMP-1, REL-1, FR-TRANSIT-1
- Technique: with the chain coupled (the launch recipe's `bind_accept … coupled` observed), one `sendsms` curl with `dlr-mask=31`; assert per hop — HTTP 202 `0: Accepted for delivery`; the `front_sms_log` `momt=MT` row (text, mask); the DLR reservation rows (`front_bearer_dlr`, `smsc_bearer_dlr`); `/metrics` `relay_pdus_total{INGRESS}` +1 (the `submit_sm`) then `{EGRESS}` +1 (the `submit_sm_resp`); opensmppbox's `submit_sm`/`submit_sm_resp` PDU dumps (the issued `message_id`); fakesmsc's receipt of the INTACT text; the DLR `deliver_sm` (`stat:DELIVRD`, `receipted_message_id` == the issued `message_id`) crossing back EGRESS with the front's `deliver_sm_resp` ROK relayed INGRESS; `removing DLR from database` + the two `momt=DLR` rows; both Kannel status pages' per-SMSC counters.
- Tooling: docker compose + curl + `docker compose logs` (log-level 4 = full SMPP PDU dumps on BOTH ends — a two-ended wire tap AROUND the proxy: what left the front vs what the SMSC got is the verbatim-forward check without touching the proxy) + psql + the proxy's `/metrics`; the TRACE run arg (`--logging.level.smpp.companion.proxy.relay.pdu=TRACE`) shows the exact framed bytes (bind family redacted at every level).
- Notes: The real-stack flavor of the transit-integrity guarantee. Submit lost/duplicated/corrupted at any hop = a REL-1 defect → bounced to the owning epic, never tuned away.
- **Status (Story 6.3, executed live 2026-09-17; row landed at T4):** every observation in `sandbox/README.md` §6.1's hop table held on the live rig — `message_id e82bd7c8` issued by opensmppbox and returned as `receipted_message_id`, `stat:DELIVRD`, the `hello` text intact at fakesmsc, front `sent: sms 1 / rcvd: dlr 2`, FAKE1 `sent: sms 1 / rcvd: dlr 1`, the TRACE arm showing `relayed pdu: direction=INGRESS command_id=0x4 length=66 …` with the message text readable in the hex and ZERO bind-family pdu lines. No transit-integrity defect found (nothing to bounce).

**E2E-003** `[manual rig — Story 6.3]` — `deliver_sm` MO injection toward the ESME: the A-1 session-affinity property against a real stack
Level: e2e · Priority: P2 · Risks: R33, R38 · NFR: COMP-1, REL-4, FR-TRANSIT-2
- Technique: with the chain coupled, inject an MO at the fake center's tty-attached stdin (`<sender> <receiver> text <message>` — the coding keyword is required; the BEARERBOX is the syntax parser, fakesmsc accepts anything); assert the MO becomes opensmppbox's `deliver_sm` toward the ESME, crosses the relay on the SAME coupled pair that submitted the MT (+1 `relay_pdus_total{EGRESS}` then +1 `{INGRESS}` for its resp), lands on the front bearerbox with the IDENTICAL `short_message` octets in its own PDU dump, and the front's `deliver_sm_resp` ROK relays back; the front status page's `rcvd: sms` +1. Requires the rig's `smsbox-route` conf delta (Kannel's bearerbox broadcasts MOs only to boxes WITHOUT a boxc-id; opensmppbox registers as `usr1`).
- Tooling: `docker compose attach` (a terminal is required; scripted feeding via a fifo held through `script -qec "docker attach …"`) + the two-ended PDU dumps + `/metrics`.
- Notes: The single-pair rig demonstrates the property; the multi-pair falsification stays the A-1 carrier plan's (OBS-035..037) — this is the real-stack approximation beside OBS-038's jSMPP one. Wrong-leg delivery = the affinity defect class → bounce.
- **Status (Story 6.3, executed live 2026-09-17; row landed at T4):** `hello from the mobile` injected at fakesmsc arrived at the front with identical octets (`source_addr:"79033374423"`, `destination_addr:"79876543210"` in opensmppbox's `Sending PDU:` dump), `deliver_sm_resp` ROK, metrics +1 EGRESS +1 INGRESS (`sandbox/README.md` §6.2). The journey itself surfaced the rig's one conf gap — the first injection died queued with `WARNING: smsbox_list empty!` until the `smsbox-route` group was added (README deltas item 4). No affinity defect found.

**E2E-004** `[manual rig — Story 6.3]` — `enquire_link` keepalive crossing the coupled pair: post-couple opaque relay, never synthesized
Level: e2e · Priority: P2 · Risks: R33 · NFR: COMP-1, FR-TRANSIT-1
- Technique: passive timed observation — launch, couple, watch Kannel's SMPP client enquire every `enquire-link-interval` (30 s default); assert each interval +1 on BOTH `relay_pdus_total` legs (the enquire INGRESS + its resp EGRESS — the tag follows the arrival leg), the mirror PDU pair in both boxes' dumps (opensmppbox answers `enquire_link_resp`; the proxy NEVER does), and the session's online-time growing without bound across idle windows.
- Tooling: `/metrics` deltas + the two-ended PDU dumps + the front status page.
- Notes: AD-3/AD-32 on a real stack: post-couple keepalive is opaque relay both ways — no proxy response synthesis, ever; PRE-couple any non-bind PDU (an `enquire_link` included) is bare-closed. The keepalive-vs-`pre-couple-idle-timeout` constraint (the client's keepalive interval must exceed its bind latency; here the 4 s adjudication deadline ≪ the 30 s interval, and Kannel enquires only on a BOUND session) is the accepted-risk interop note, stated with the live arithmetic in `sandbox/README.md` §6.3.
- **Status (Story 6.3, executed live 2026-09-17; row landed at T4):** first `enquire_link` at couple + 30 s, then one per ~30 s, both metric legs ticking in step, session online-time unbounded — observed as written (`sandbox/README.md` §6.3).

**E2E-005** `[manual rig — Story 6.3]` — The two deny journeys against the real stack: AD-32 case-4 verbatim non-ROK forward vs AD-33 collapsed generic deny — where each failure surfaces (wire vs log vs metric)
Level: e2e · Priority: P2 · Risks: R32, R33 · NFR: COMP-1, REL-1, FR-SEC-5
- Technique: D1 (wrong SMSC credential — ROPC-valid at Keycloak, absent from opensmppbox's `smpp-logins`): the SMSC's own non-ROK `bind_transceiver_resp` forwards VERBATIM to the front (AD-32 case 4 — the SMSC is the sole credential authority), both legs torn down; proxy stdout SILENT (no verdict was returned — AD-27's pinned triggers), `relay_connections_closed_total{…BIND_FAILED_NON_ROK}` +1 per retry on BOTH legs, `relay_binds_*` flat. D2 (bad OIDC credential — valid SMSC user, wrong password at Keycloak): `bind_reject` `verdict":"DenyInvalid"` per retry with `0x0000000D` on the wire, `relay_binds_rejected_total` AND `relay_binds_unknown_total` climbing, closes `{INGRESS,BIND_REJECTED}` only, ZERO `bind_transceiver` at opensmppbox (the deny fired before the forward). Both arms: the front's wire line is the IDENTICAL `code 0x0000000d (Bind Failed)` — one generic code (AD-33), the rich reason in the proxy's JSON logs only.
- Tooling: per arm a front-conf credential edit + `docker compose restart front-bearer-box` (restore the same way); the three surfaces read per `sandbox/README.md` §6.4's table (front wire log, proxy stdout, `/metrics`, opensmppbox's dump).
- Notes: The deny journeys double as the sandbox's most valuable debugging content — WHERE each failure surfaces. Rig honesty: opensmppbox itself answers `0x0000000d` on credential refusal, so in THIS rig the two arms are wire-identical down to the code; the discriminating surfaces are opensmppbox's own PDU dump (its answer, un-collapsed, `system_id: NULL` and all) and the metric shape (`BIND_FAILED_NON_ROK` both legs = the SMSC answered; `BIND_REJECTED` ingress-only = the proxy answered) — stated in the README, not papered over. The launch-recipe's failure arms (§5.4: wrong egress port, stale secret, Keycloak-down — the last two wire-indistinguishable per AD-33) complete the deny-surface set.
- **Status (Story 6.3, executed live 2026-09-17; row landed at T4):** both arms and both restores executed live; each restore re-coupled within one 10 s retry (bind_accept #3/#4). A collapsed or synthesized response on D1's wire would violate the verbatim contract — none observed (`sandbox/README.md` §6.4).
...
- **A-1 is unfalsifiable in CI.** The in-JVM mock SMSC EMULATES A-1 (assumes it); the only genuine A-1 gate is the non-CI real-carrier ops plan (OBS-035..038). OBS-038 (jSMPP server-side mock) is the strongest CI approximation. *(Story 6.3, 2026-09-17: the Kannel manual rig's E2E-003 adds the real-stack, non-CI approximation — a `deliver_sm` injected at the SMSC side riding the relay back to the originating front bind — single-pair; the multi-pair falsification stays the carrier plan's.)*

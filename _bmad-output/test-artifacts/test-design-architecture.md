...
| **R6**     | PERF     | PERF targets overclaimed / methodology bar (self-measured, first-of-kind, 4 regimes).                                             | 2 | 3 | 6       | Honest harness methodology + no-crypto baseline; publish saturation knee + failure modes (R25/R26 support).                               | Author    | Epic 7 (2026-09-12 split) |
...
| **R25**    | PERF     | Load-gen coordinated omission / open-vs-closed model (from-scratch, no reference) invalidates PERF-1 percentiles.                 | 2 | 3 | 6       | Open-model load harness; load-gen on separate cores; no coordinated omission (harness is itself an Epic 7 deliverable — 2026-09-12 split).| Author    | Epic 7 |
...
3. Add a race-oriented soak (delay-injection / event-loop-jitter / randomized `channelInvalid`) distinct from the Epic 7 throughput soak (re-pointed 2026-09-12).

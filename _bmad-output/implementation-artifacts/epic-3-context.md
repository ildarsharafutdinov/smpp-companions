...
Configure per-leg TLS by mode and delegate legacy password-grant validation to an operator-run OIDC provider (Keycloak ROPC), so the proxy fail-closed-denies every bind on indeterminate verdicts, holds zero persisted credentials, and preserves `system_id` end-to-end. The adjudication half is complete (3.1 probe: MET; 3.2: production ROPC adapter behind the ratified-immutable port). The remaining work is the TLS half — Modes A/B/C on the internet leg, the forward-role acceptor, and the routing-miss deny arm — which closes the two-proxy transit-security story and unblocks Epic 4.
...
- Story 3.3: TLS modes A/B/C + forward-acceptor widening (not yet created — the remaining scope)

## Requirements & Constraints

- **Mode semantics (amended two-proxy topology).** Mode A = one-way TLS on the internet leg; Mode B = plaintext internet leg, **reverse-only** (forward×B rejected at startup; the legacy client connects directly to the reverse, which still adjudicates) — loud startup warning + explicit opt-in ack, then starts; Mode C = mTLS on the internet leg. The reverse→SMSC leg is **plaintext in every mode**; the forward's legacy leg is plaintext (trusted network is the sole gate — no local password check or store).
...
- **Accepted risks constraining scope:** Mode A two-proxy without ACL isolation (forward cannot authenticate the reverse in one-way TLS — loud warning) and Mode B plaintext passwords over the internet (opt-in).
- **Bind latency budget:** p99 ~250 ms warm, ≤2 s cold, fail-closed DENY beyond a 2–5 s timeout.

## Technical Decisions

- **Reverse-adjudicates topology (amended 2026-08-18; supersedes older "forward adjudicates" wording still in story-3.2 AC prose and the unswept walkthrough narrative — the spine + memlog are authoritative).** The reverse is the sole enforcement point (ROPC before anything reaches the SMSC); the forward is a trusted-side relay with no OIDC material. TLS contexts to build: the forward's internet-leg **server** context (Mode A/C; Mode C adds client-auth REQUIRE) and the reverse's internet-leg **client** context (validates the forward's server cert against the operator trust store; Mode C presents the per-instance client cert). No TLS on the legacy or SMSC legs.
...
- **Egress endpoint identification:** on the reverse→forward client connect, set `endpointIdentificationAlgorithm` explicitly (`null` for raw-IP targets, or IP-SAN certs with verification on) — Netty 4.2 defaults it to HTTPS and fails raw-IP connects. The reverse→IdP link is JDK `HttpClient` with verification always on.
...
- **Routing is 1:1 in v1:** a `system_id` allow-list mapping to the single egress; value schema `{host, port, tlsContextId?}` with a `tls.contexts` map for per-target Mode C certs.

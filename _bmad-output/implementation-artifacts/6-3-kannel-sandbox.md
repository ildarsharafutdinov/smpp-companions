...
- `proxy/src/docker/Dockerfile` + `buildSrc/src/main/kotlin/smpp/deploy/{AssembleDockerContextTask,DockerImageTask}.kt` -- the distroless image the T5 docker combos run (built by the existing Gradle docker tasks — no build-file change); the image's jar path is the bind-mount target that makes the container jar-replaceable.
- `proxy/src/test/java/smpp/companion/proxy/testsupport/DockerRig.java` (`suiteImage`, `launchComposedModeCChain`) + `bootstrap/ComposedDockerE2eTest.java` -- the PROVEN docker shapes for the composed cells (mode C chain, single reverse) and the committed TLS material the mode A/C runbooks copy from the test tier.
- `docs/configuration.md` "The role × mode matrix" -- the cell table (required vs structurally-absent keys per cell) the runbooks' arg sets mirror; `docs/deployment-guide.md` -- the canonical `docker run` recipe + the Mode A/B/C sections the runbooks simplify from.
...
- Given any of the three combo runbooks, when followed verbatim from a clean checkout with Docker, then the rig reaches healthy and that combo's chain couples (`bind_accept` observed), with every expected observation and log location in the runbook holding as written.

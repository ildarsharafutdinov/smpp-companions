...
- source_spec: none
  summary: Make the public docs human-friendly and readable — a distant-future readability pass over the `docs/` pages (English originals and their `docs/ru/` translations). Today they are written in the repo's review-grade register — dense cross-reference chains, inline AD-xx/SEC-xx/Story tag citations mid-sentence, long parentheticals, high terminology load — which serves the page↔reality review duty but is hard reading for an operator meeting the proxy for the first time. Candidate shape: plain-language leads (what/why before the oracle-grade detail), progressive disclosure of the ratification tags, worked per-variant examples, separating "what an operator needs now" from "ratification archaeology" — WITHOUT dropping the normative content (oracle pointers, verbatim refusal strings, the AD-30 interlock formula) or breaking the review-coherence rule (a key change goes through a story that touches the page; nothing in the repo parses it).
  evidence: Owner note 2026-09-13, surfaced by the docs/ru translation + terminology pass (the repeated hand-edit rounds kept simplifying register — variant for cell, connection for dial, intentional for deliberately, parameter for knob, set for suite — indicating the desired plainness level the originals do not yet meet). Timing: distant future by owner decision — after the doc set is complete (it completed 2026-09-13 with Story 6.1 — only Epic 7's perf-report page remains outstanding) and Epic 7 publishes the perf numbers, so the pass reworks a complete, stable surface once instead of churning with each landing.

## Deferred from: code review of 6-2-composed-packaged-conformance (2026-09-16)

- source_spec: `6-2-composed-packaged-conformance.md`
  summary: No standing "one DockerRig" structural guard — T1's BH7 sweep (manual greps + a planted-then-reverted re-duplication) enforces nothing going forward; a fourth consumer could re-introduce a private rig-family copy silently, the exact drift failure mode BH7 recorded. Candidate shape: the `ConnectionRegistryMutationFenceArchitectureTest` forbid+positive-control idiom pinning "one `class DockerRig`, zero folded-family private definitions outside `testsupport/`".
  evidence: Code review 2026-09-16 (blind-hunter + acceptance-auditor layers, source-verified: no such rule exists; the folded-family grep runs only in the story's Debug Log). Deferred: pre-existing posture — the 4.2/4.3 fixture folds (`TokenIdpStandIn`, the parked-IdP consolidation) carry no standing guard either; pick up if/when a fourth rig consumer nears.
- source_spec: `6-2-composed-packaged-conformance.md`
  summary: The daemon-less expectation of AC-8 and `docs/deployment-guide.md` § conformance run ("without a daemon the Docker rung skips with its stated reason and the other two rungs still run — 5 rows") has never been observed on this machine: `~/.testcontainers.properties` pins `docker.client.strategy=UnixSocketClientProviderStrategy`, so both override attempts reached the real socket (the Dev Record discloses this). The `@Testcontainers(disabledWithoutDocker = true)` attribute is present on all three Docker suites and the skip mechanism is standard Testcontainers behavior, so the claim is credible — just never executed.
  evidence: Code review 2026-09-16 (acceptance-auditor + verification-gap layers, corroborating the Dev Record's own disclosure at the T2 daemon-less entry). Deferred: settles the first time the build runs on any daemon-less machine; no change is possible or needed on this box.
- source_spec: `_bmad-output/implementation-artifacts/6-3-kannel-sandbox.md`
  summary: Sandbox rig supply-chain hardening + the never-exercised cold-build path — `postgres:latest` floats while everything else is pinned; the Kannel source tarball is fetched `curl -k` with no checksum; exact PGDG RPM URLs (devel 15.15 vs runtime libs 15.17) will rot; the "clean checkout" clauses of the story's AC lines 88/93 were proven warm-machine only (T1's full layer-cache hit, disclosed in its completion notes).
  evidence: Frozen decision ports the playground Dockerfile byte-for-byte, so hardening exceeds the story's ratified shape; settling evidence = one `docker build --no-cache` of the kannel image + a fresh-checkout runbook bring-up to `bind_accept coupled` (would also settle the clean-checkout AC clause); pin postgres to the tested version and add a §10 deltas note.
- source_spec: `_bmad-output/implementation-artifacts/6-3-kannel-sandbox.md`
...
- source_spec: `_bmad-output/implementation-artifacts/8-1-observability-audit-and-gap-close.md`
  summary: The dead-peer drop branch's zero-fire is unpinned — no test drives a PDU arriving with a live coupled entry whose peer channel is already inactive, so the runbook's explicit drop-case promise ("a PDU dropped to a dead peer records nothing — nothing was relayed", `docs/runbooks.md` `relay_pdus_transit_seconds` row) and its identical non-fire for the pre-existing `relay_pdus_total{direction}` counter ride unobserved; hoisting the transit record above the dead-peer check (`CoupledRelayHandler` ~:193-203) would pass every suite.
  evidence: Verification-gap layer (pre-verified as filed): repo-wide search of fire-count emptiness pins finds five hits, all pre-couple/violation rows; the nearest (`RelayIngressHandlerTest` RELAY-009(b)) drives the entry-absent prelude branch instead, and the EmbeddedChannel harness cannot reach the dead-peer branch today — closing the peer fires `channelInactive` → teardown removes the entry first. Settling evidence = a fixture that suppresses the peer's entry teardown before close so the entry survives (then: fire `onFramedPdu` zero times, `relay_pdus_transit_seconds_count` and `relay_pdus_total` untouched); deferred because the race-window fixture is new rig work and the counter twin was already unpinned before this story.
- source_spec: `_bmad-output/implementation-artifacts/8-1-observability-audit-and-gap-close.md`
  summary: Spine AD-27's live trigger sentence still lists "`kid`-miss" among non-verdict denial causes routed to the unlabeled counter — dead vocabulary since Story 3.4 T1+T2 (2026-08-27) killed local JWT verification; the spine's own AD-33 (~:258) records the cause's death.
  evidence: Blind-hunter layer (source-verified against both spine sites); harmless as a rule — a dead cause trivially never routes through `onBindReject` — but freshly amended normative text carrying a concept the same document declares dead invites reader confusion. Pre-existing (the clause predates this story; T6's amendment rewrote the surrounding enumeration, not this sentence); sweep at the next full spine re-distill, which the frozen epics `[AD-N]` mirror bullets are already flagged for.

## Deferred from: code review of 8-2-sandbox-prometheus (T4 round, 2026-09-21)

- source_spec: `_bmad-output/implementation-artifacts/8-2-sandbox-prometheus.md`
  summary: A8's corrected `_max`-inclusive timer fan-out (124 forward / 123 reverse, the arithmetic of record for the next live conformance check) is pinned by no test — `MetricsEndpointTest.scrapeCarriesBothHistogramFamiliesPreRegistered` still asserts only `_bucket`/`_count`/`_sum` rows, the exact undercount the T2 correction fixed.
  evidence: Verification-gap layer (pre-verified as filed): repo-wide `grep -rn '_max' --include='*.java'` over `proxy/src`, `buildSrc/src`, `codec/src` returns zero; the scraped body in that very test already contains the `_max` rows unasserted (`PrometheusMeterRegistry` rendering; the T1 idle count of 37 rows across both families only adds up with one `_max` per timer). A Micrometer/registry change dropping `_max` stays GREEN across all 550 tests while the recorded baseline silently disagrees — surfacing only at the next manual live check. Cannot land inside 8.2 (frozen AC4: the story's whole diff touches nothing under `proxy/`); a standalone test-only commit or Story 8.3 carries it — extend the histogram assertions with the zero-valued `_max` rows in the existing assertion style.
- source_spec: `_bmad-output/implementation-artifacts/8-2-sandbox-prometheus.md`
...
## Deferred from: code review of 8-2-sandbox-prometheus (story-close round, 2026-09-21)

- source_spec: `_bmad-output/implementation-artifacts/8-2-sandbox-prometheus.md`
  summary: The always-configured 9091 target's UP arm is config-derived only — every live observation to date (T1 proof, T2 A8 check) ran the rig's default single-instance reverse posture, so README §5.6's "9091 UP while a two-instance runbook pair runs" sentence (round-2 B11 markers, EN+RU) and the a/c runbook rows have never been observed live.
  evidence: Blind-hunter layer (story-close round, verified against the T5 note's own honesty scope and the A8 bullet): the pend lived only in the in-prose "first live confirmation pending" markers — no ledger row, while the same-class `_sum` identity pend has one. Settle by running any two-instance runbook pair (§5.6 a or c) and observing the 9091 target UP + both instances' series in the TSDB; confirm or annotate the §5.6 sentence and runbook rows then.
- source_spec: `_bmad-output/implementation-artifacts/8-2-sandbox-prometheus.md`

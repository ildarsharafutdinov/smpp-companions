...
- **Epic 6 depends on this epic:** formal performance validation and final conformance run on the PACKAGED shapes; the operator docs surface must state plainly that v1 authenticates via ROPC (a grant on a removal track) and that operators must pin their Keycloak build. Deferred to Epic 6 by 5.2's close-out: packaged-shape e2e beyond the reverse-B allow journey (mode A/C cells and auth-DENY journeys have no e2e row in either shape — see the deferred-work ledger).

...
1. **[AC1] Green build on pinned JDK 25 + `--enable-preview`.** `./gradlew clean build` succeeds on a **JDK 25.0.x** toolchain, with `--enable-preview` applied to compile, test, **and** run. **(Amended 2026-07-25 FIXME audit:)** the JDK-25 requirement is pinned via `languageVersion`; the **vendor is intentionally NOT pinned** and the build does **not** auto-provision — JDK 25 is an environment precondition (asdf `.tool-versions: temurin-25.0.3` locally; a CI setup step / image later). The earlier Eclipse-Temurin-only vendor pin, `ToolchainPinTest`, and the foojay-resolver auto-provisioning were removed. *(Maps DEPLOY-014 / SEC-085; gate-exists form — no catalog positive control for off-pin rejection.)*
...
5. **[AC5] CVE/dependency-check gate + positive control — SEC-091/099 green.** Nimbus resolves **≥ 10.0.2** (CVE-2025-53864 floor), enforced by a **deterministic dependency-floor gate wired into `check`** (`enforceDependencyFloors`) — the load-bearing control; the SEC-099 TestKit positive control injects Nimbus < 10.0.2 and asserts the build **FAILS** in an isolated Gradle run. **OWASP `dependency-check` (SEC-091) is a defense-in-depth CI lane — NOT wired into `check` and WITHOUT a positive control** (decision accepted at code review 2026-07-24; see Positive-control mechanism).
...
> Code review 2026-07-24 (`bmad-code-review`, parallel adversarial layers: blind + edge-case + acceptance-audit). **Caveat:** same-model session (glm-5.2) that authored the code — not the fresh-context + different-LLM review BMad recommends; leaned on parallel adversarial subagents to compensate. Triage: **3 decision-needed, 8 patch, 7 defer, 5 dismissed. No HIGH/intolerable defects** — the substrate (modules, JDK pin, gates, non-web bootstrap, license) is sound.
>
> **Resolution (2026-07-24):** all 3 decisions resolved (D1→applied: AC5/Dev-Notes/Completion-Note wording reconciled to the two-lane reality; D2→deferred to Story 1.3, AD-30 claim struck + logged; D3→accepted). All **8 patches applied + verified** — `./gradlew clean build :buildSrc:test` → BUILD SUCCESSFUL, **22 tests green** (9 buildSrc + 3 codec + 10 proxy), gates CODEC-040/SEC-099/OBS-013 green on the real modules. 7 defer logged to `deferred-work.md`; 5 dismissed as noise. Patch changes are uncommitted as of this review.

**Decision-needed (resolve first):**

- [x] [Review][Decision ✅ APPLIED 2026-07-24: option (a) — floor gate kept as load-bearing control; AC5 + Dev-Notes Positive-control + Completion Note reconciled to the two-lane reality; residual OWASP-suppression-bypass risk accepted] **SEC-099 drives the floor gate, not the OWASP lane; AC5 text says "OWASP runs in the build"** — Spec's Positive-control mechanism names the OWASP dependency-check task for SEC-099; impl drives `enforceDependencyFloors` instead, leaving SEC-091 OWASP with **no positive control**. AC5 literal text ("OWASP dependency-check runs in the build") is false (OWASP is a CI lane, not in `check`). `failOnUnscored` unset → OWASP can't catch CVE-2025-53864 (NVD-unscored). The Nimbus ≥10.0.2 invariant IS still enforced (deterministic floor gate in `check` + its TestKit control) — security floor is real; the gap is spec wording + OWASP-lane efficacy. Options: (a) accept two-lane split + update AC5 text to reality; (b) add hardening — assert no OWASP suppression files + set `failOnUnscored=true`; (c) wire OWASP into `check` + its TestKit control (slow/flaky). [proxy/build.gradle.kts:34-44; Dev Notes §Positive-control mechanism; AC5]
- [x] [Review][Decision ✅ DEFERRED→Story 1.3 2026-07-24: AD-30 Dev-Notes claim struck + logged to deferred-work.md; reason: config-matrix story owns all `companion.*` keys together] **AD-30 config formula-inputs missing (`max_frame`, `max_inbound_depth`, `concurrent_pairs`, `safety_factor`)** — Dev Notes (AD-30) name these four keys as THIS story's sole AD-30 deliverable in the `companion.*` skeleton; none appear in `application.yml` or `CompanionProperties`. Undisclosed in Completion Notes. Options: (a) add the four keys now (per Dev Notes); (b) defer to Story 1.3 with the config matrix. [Dev Notes AD-30; application.yml; CompanionProperties.java]
- [x] [Review][Decision ✅ ACCEPTED 2026-07-24: deferral is disclosed in Completion Notes; owning stories pin versions] **"Declare now, exercise later" deps not declared (jSMPP 3.0.2 / JQF / jqwik / JMH / BlockHound)** — Dev Notes say "Declare now"; impl deferred ALL (disclosed in Completion Notes as "optional"). jSMPP rationale (unused dep) is reasonable; the others are weaker — mind the jSMPP-2.3.11 trap. Options: (a) accept deferral (fine default); (b) pin jSMPP 3.0.2 (+ others) now without wiring harnesses. [Dev Notes §Dependencies]

**Patch (unambiguous fixes):**

- [x] [Review][Patch] **Positive controls don't assert the gate is wired into `check`** — TestKit tests invoke the gate task directly; deleting any `tasks.named("check"){dependsOn(...)}` line leaves all controls green while the gate silently stops running on `./gradlew clean build` (the exact silent-bypass the spec's RELAY-018 note warns of). Add a `runner("check")` buildAndFail case per gate. [buildSrc/src/test/kotlin/.../gates/*.kt]
- [x] [Review][Patch] **`appRefusesToStartWithInvalidRole` asserts `Exception.class` (too loose); absent-role-through-Spring untested** — AC9 covers absent AND invalid; the invalid Spring-path is over-loose, the absent path is only ctor-unit-tested (bypasses binding). Tighten via a message check (version-robust) + add an absent-through-Spring case. [proxy/src/test/.../config/CompanionRoleFailFastTest.java:47-51]
- [x] [Review][Patch] **`companion.tls.*` relaxed binding asserted by inspection only** — AC9 requires relaxed-binding of the key shapes; nothing boots/loads `application.yml` and asserts `tls12CipherSuites` is populated. Add a bind/context test. [no test; AC9]
- [x] [Review][Patch] **OBS-013 enumeration gaps — Jetty/Undertow starters slip through** — `spring-boot-starter-jetty`/`-undertow` (and jersey/armeria) aren't forbidden; either ships an embedded server while passing the gate (AD-16 intent). Add the alternate embedded-server starters/groups. [buildSrc/src/main/kotlin/smpp.runtime-purity.gradle.kts:16-25]
- [x] [Review][Patch] **CODEC-040 inspects only `compileClasspath`** — a `runtimeOnly` non-netty dep in codec evades it (AC4 says "compile classpath" literally, but AD-7 purity is broader). Also scan `runtimeClasspath`. [buildSrc/src/main/kotlin/smpp.codec-purity.gradle.kts:15]
- [x] [Review][Patch] **`mavenCompare` silently coerces non-numeric segments to 0** — `10.0.2-RC1`→`10.0.0`, `RC1`==`RC2`. Latent (one clean-numeric floor today) but fails-open, against the fail-closed ethos. Fail loud on a non-numeric segment on a gated coordinate. [buildSrc/src/main/kotlin/smpp.dependency-floors.gradle.kts:49]
- [x] [Review][Patch] **`ToolchainPinTest` accepts vendors the pin excludes** — `containsAnyOf(...,"AdoptOpenJDK",...)` over-accepts a dead pre-Eclipse distro that `ADOPTIUM` never selects; real Temurin reports "Eclipse Adoptium". Narrow the assertion. [proxy/src/test/.../bootstrap/ToolchainPinTest.java:24]
- [x] [Review][Patch] **`corpusScaffoldIsInPlace` is a no-op test (asserts nothing)** — inflates the "17 green" metric. Empty corpus is correct for 1.1 (AC10), but make the no-op explicit: `@Disabled` + 1.2 ref, or an explicit `assertThat(count).isGreaterThanOrEqualTo(0)`. [codec/src/test/.../golden/GoldenVectorCorpusTest.java:44-48]

**Defer (real, by-design / future-story — logged to `deferred-work.md`):**

- [x] [Review][Defer] **CODEC-039 / SEC-090 ArchUnit rules have no positive control today** — scaffold form explicitly accepted by AC3/AC7 ("locks the boundary before code exists"); SEC-090 narrower than AC7 text (PKIX / `javax.net.ssl`, not just `sun.security`) tightens in Epic 3 (disclosed). [codec + proxy ArchUnit tests]
- [x] [Review][Defer] **`--enable-preview` COMPILE/RUN not runtime-verified** — wiring exists in the conventions plugin; only the test-JVM path is asserted (disclosed). Future TestKit compile-arg hardening. [EnablePreviewArgTest.java; Completion Notes (5)]
- [x] [Review][Defer] **`CompanionProperties` has no `tls`/`tls.protocols` null guards** — only `role` is fail-fast-guarded, exactly AC9's scope (matrix is 1.3). tls-absent NPE is a 1.3 concern. [CompanionProperties.java:23-28]
- [x] [Review][Defer] **`contextCloseStopsLifecycleWithinGracefulTimeout` 30s-ceiling assertion is trivial** — the test DOES verify stop() ran (`isRunning` false); the upper-bound passes trivially on the stub. Real AD-22 body + meaningful timeout test is Epic 4. [BootstrapLifecycleTest.java:36-48]
- [x] [Review][Defer] **`BootstrapLifecycleTest` non-web assertion is tautological (forces `.web(NONE)`)** — AD-16 is genuinely guarded by OBS-013 + `spring.main.web-application-type:none`; the class-name check can't detect classpath drift. Cosmetic. [BootstrapLifecycleTest.java:29,52]
- [x] [Review][Defer] **`CompanionLifecycle` phase ordering (default MAX_VALUE stops first) once a 2nd `SmartLifecycle` lands** — single bean today; Epic 4 manages phases. [CompanionLifecycle.java]
- [x] [Review][Defer] **SEC-099 has only a Nimbus floor — other deps unchecked** — AC5 is Nimbus-specific; the OWASP lane is the general scanner. Maintenance concern, not a 1.1 defect. [smpp.dependency-floors.gradle.kts:8-10]

> Dismissed as noise (5): `.gitignore` `!gradle-wrapper.jar` negation with no preceding exclude (no effect); `CompanionApplication` placed in proxy root vs `.bootstrap` (justified — component-scan coverage, documented in Javadoc); OBS-013 test-scoped transitives (not production runtime); `CompanionLifecycle` null-callback NPE (Spring's `stop(Runnable)` contract guarantees non-null); JUnit BOM 6.0.3 vs Dev Notes "JUnit 5" (JUnit 6 platform is current in 2026, Jupiter API unchanged).

### Review Findings — FIXME-mark audit (2026-07-25)

> Follow-up `bmad-code-review check FIXME marks` (ultracode: 3 parallel adversarial lenses → perspective-diverse refutation per finding; same-model glm-5.2 session compensating with adversarial depth). Target: **5 working-tree-only `// FIXME` build-file comments** (none in HEAD `8b78dab`; added after the 2026-07-24 review). Verified unanimous verdict — no FIXME should be acted on. Resolved per owner decision; build green (21 tests).

- [x] [Review][Decision ✅ APPLIED 2026-07-25: owner chose to RELAX AC1] **Toolchain-pin FIXMEs resolved by relaxing the pin, not by keeping it.** Removed the Eclipse-Temurin vendor pin (`vendor.set(JvmVendorSpec.ADOPTIUM)`) + `ToolchainPinTest`, and removed `foojay-resolver-convention` entirely. `languageVersion=25` retained: JDK 25 is now an environment precondition (asdf `.tool-versions: temurin-25.0.3` locally; a CI setup step/image later), vendor not pinned, build does not auto-provision. AC1 amended above. *(Reviewer note: this reverses the AC1 "non-silently-drifting vendor" guarantee — accepted by owner; the JDK-25 version pin + AD-5 preview-semantics rationale remain.)* [java-conventions.gradle.kts; settings.gradle.kts; AC1; AD-5; DEPLOY-014; SEC-085]
- [x] [Review][Patch ✅ APPLIED] **Removed the two `// FIXME: prefer archunit` notes** (codec + runtime gates), replaced with accurate "Gradle resolution gate, NOT ArchUnit" pointers. Both gates (CODEC-040, OBS-013) kept verbatim — ArchUnit categorically cannot express Maven-group granularity, classless aggregator POMs, or the compile/runtime classpath split. [codec-purity.gradle.kts; runtime-purity.gradle.kts]
- [x] [Review][Patch ✅ APPLIED] **Removed the redundant `// FIXME: CVE analysis will run separately`** — duplicates the file header (lines 1-5). SEC-099 floor gate + `DependencyFloorsGateTest` kept (AC5 intact). [dependency-floors.gradle.kts:8]

> **Resolved (owner: "act now") — the 4 owner-added proxy `// FIXME` comments:** all four removed. Naming theme → renamed `CompanionApplication`/`CompanionLifecycle`/`CompanionProperties` to the `Proxy*` convention ("use ProxyCompanionApplication" + "adjust with application name"). Validation theme → migrated AC9 fail-fast to Spring `@Validated`/`@NotNull` ("prefer spring validation"), refined OBS-013 to allow the `tomcat-embed-el` EL impl (not a server). See Change Log (2026-07-25). ZERO `// FIXME` markers remain in source.

## Dev Notes

### Tech Stack & Versions (pinned — use verbatim)

- **JDK** — Eclipse Temurin **25.0.x LTS**, `--enable-preview` process-wide (build+test+run). Rationale: `StructuredTaskScope` is preview-only on JDK 25 (JEP 505); the whole runtime runs under preview semantics once STS lands, so pin the exact build now so toolchain drift cannot silently change preview semantics. [SPINE AD-5, Stack] **(2026-07-25 FIXME audit: the VENDOR pin was relaxed — only the JDK-25 version is pinned now; vendor is env-supplied via asdf. See the AC1 amendment.)**
...
- **AD-30** — one named `max_command_length` constant + `MaxDirectMemorySize` formula. **The codec constant is deferred to Story 1.2** (nowhere to live in an empty codec); the **config formula-inputs** (`max_frame`, `max_inbound_depth`, `concurrent_pairs`, `safety_factor`) are **deferred to Story 1.3** with the rest of the config matrix (decision at code review 2026-07-24 — not added in 1.1).
...
2. invokes the **gate Gradle task** in that forked build (the CODEC-040 dep-allowlist task, **the SEC-099 deterministic dependency-floor task `enforceDependencyFloors`** — NOT the OWASP dependency-check task, which is a defense-in-depth CI lane without a positive control; accepted at code review 2026-07-24 — the OBS-013 forbidden-runtime-dep scan);
...
- `proxy/src/main/java/smpp/companion/proxy/ProxyCompanionApplication.java`
- `proxy/src/main/java/smpp/companion/proxy/bootstrap/ProxyCompanionLifecycle.java`
- `proxy/src/main/java/smpp/companion/proxy/config/ProxyCompanionProperties.java`
- `proxy/src/main/java/smpp/companion/proxy/{bootstrap,config,relay,security,observability}/package-info.java`
- `proxy/src/main/resources/application.yml`
- `proxy/src/test/java/smpp/companion/proxy/bootstrap/{BootstrapLifecycleTest,EnablePreviewArgTest}.java`
...
- 2026-07-25 — FIXME-mark audit (follow-up `bmad-code-review check FIXME marks`): removed the 5 working-tree build-file `// FIXME` notes. Kept CODEC-040 / OBS-013 purity gates (with accurate "Gradle resolution gate, not ArchUnit" pointers) and the SEC-099 dependency-floor gate + its TestKit control (only the redundant note was removed; AC5 intact). **Relaxed AC1:** dropped the Eclipse-Temurin vendor pin (`vendor.set(JvmVendorSpec.ADOPTIUM)`), deleted `ToolchainPinTest`, and removed the `foojay-resolver-convention` plugin entirely (no auto-provisioning); `languageVersion=25` retained — JDK 25 is now an environment precondition (asdf `.tool-versions` locally), vendor not pinned. `./gradlew clean build` + `:buildSrc:test` → BUILD SUCCESSFUL, 21 tests green (codec 3 + proxy 9 + buildSrc 9).
- 2026-07-25 — Proxy-source `// FIXME` resolution (owner: "act now"): renamed the entry + companion classes to the `Proxy*` convention — `CompanionApplication`→`ProxyCompanionApplication`, `CompanionLifecycle`→`ProxyCompanionLifecycle`, `CompanionProperties`→`ProxyCompanionProperties` (all references updated; `@ConfigurationProperties("companion")` prefix unchanged). Migrated the AC9 fail-fast from the manual compact-constructor guard to Spring bean validation: `@Validated` + `@NotNull` on `role` (+ `@Valid` on `tls`), added `spring-boot-starter-validation`; removed the obsolete `recordGuardRejectsAbsentRole` unit test (absent-role is now covered by the Spring-binding test, which passed → `@NotNull` fires end-to-end). Refined OBS-013 (AD-16): `org.apache.tomcat.embed` group ban → per-artifact ban on `tomcat-embed-core`/`-websocket` (the embedded server) so the `tomcat-embed-el` EL impl pulled by Hibernate Validator is allowed while the server stays excluded. `./gradlew clean build` + `:buildSrc:test` → BUILD SUCCESSFUL, 20 tests green (codec 3 + proxy 8 + buildSrc 9). ZERO `// FIXME` markers remain in source.

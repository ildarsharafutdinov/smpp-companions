...
- **Ratification (AC8, 2026-08-10):** the `BindCredentialVerifier` port contract above is **ratified and immutable henceforth** — validated, *before `relay/` commits against it* (Story 2.1, the Epic 2 opener), across **all four AD-12 paths** against a pinned Keycloak ≥26.7.0: (1) JWT happy path → `Allow`; (2) RFC 7662 introspection `active:true` → `Allow`; (3) RFC 8705 mTLS provider auth (**no** `client_secret`) → `Allow` — the "most likely to fail" path works, so the shape can express every path; (4) ≥1 DENY branch end-to-end (4xx → `DenyInvalid`; non-4xx / timeout / network error / JWKS `kid`-miss / claim mismatch → `DenyIndeterminate`), plus `cancelHttp()` aborting the underlying `HttpClient` exchange per AD-32. **No contract-revision finding** — the `Verdict` / `BindCredential` / `VerdictRequest` / `verify(ScopedValue<RequestContext>)` shape (incl. the 2026-08-09 `Password`-over-`AsciiString` revision) expresses all four paths unchanged; the AC3 `cancelHttp()` binding was verified against the JDK 25 source (`MinimalFuture.cancel(true)` → `MultiExchange.cancel` → `connection.close()`) and needs no revision. The full production ROPC adapter stays in Epic 3 behind this **unchanged** port; the test-tier slice that ratified it (`RopcSlice`) lives under `proxy/src/test`. *Discipline: no silent closure — a path that proved the shape unexpressive would have been recorded as a contract revision here, not silently worked around.* *(2026-08-19: path (3)'s RFC 8705 mTLS provider auth stands as the ratified historical result — the test-tier slice stays green — but is no longer a production config option; see the Provider-link rule above.)*

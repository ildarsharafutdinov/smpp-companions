...
- **Data plane** (Netty event loops, platform threads): each socket leg is a pipeline `SslHandler → SmppFrameDecoder → SmppCodec → BindInterceptor → RelayHandler`. The bind→couple transition (AD-25) is a one-way flag write; post-bind the pair is coupled into a bidirectional framed-PDU relay.
...
flowchart TB
  ...
  IL -->|"on decoded bind_*_resp ROK: RelayHandler couples the pair → relay"| EL
```

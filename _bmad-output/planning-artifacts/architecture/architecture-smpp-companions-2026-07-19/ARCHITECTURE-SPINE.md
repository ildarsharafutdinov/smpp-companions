...
- **Two-proxy flow (both directions owned)** *(AMENDED 2026-08-18, user-directed — adjudication moved to the reverse role; see .memlog)*:** the **reverse role performs OIDC verification** — it is the **sole enforcement point**, adjudicating every bind's password-grant via ROPC **before forwarding anything to the SMSC** (the reverse protects the SMSC; the SMSC remains the sole credential authority). The **forward role is a trusted-side relay**: it terminates the trusted legacy leg (no TLS/mTLS/IdP there — the trusted network is the sole gate, AD-15), holds **no OIDC material**, and relays the original bind across the internet leg. The reverse proxy authenticates to the forward proxy as the TLS client. **In Mode C** the forward proxy validates the reverse proxy's mTLS client cert. **In Mode A (one-way TLS) the forward proxy CANNOT authenticate the reverse proxy** — so the forward proxy's internet-port listener **MUST be ACL-isolated to operator reverse-proxy hosts** (mirroring A-3), or **Mode C must be used for the internet leg**; a Mode A two-proxy deployment without ACL isolation is an **accepted risk** (register entry + loud startup warning — the Mode B pattern): any peer reaching the forward proxy's internet port receives the relayed original bind — **unverified credentials** under this amendment (a fake "reverse" is a password harvest point; the ROPC screen no longer sits between the forward and such a peer). The reverse proxy forwards to the SMSC **over plaintext** — the SMSC leg is a trusted network in every mode (no TLS/mTLS on that leg; the modes apply to the internet leg only). **Mode B has NO forward proxy — there is no forward×B cell:** the legacy client connects **directly to the reverse proxy** over the plaintext internet leg (accepted risk, opt-in ack); the reverse still adjudicates via ROPC before the SMSC — "plaintext + ROPC," a strengthening vs the pre-amendment model where Mode B had no OIDC anywhere.
...
| --- | --- | --- | --- |
| **forward** | server cert+key (internet leg); routing table; **no OIDC — trusted-side relay (AD-12 amended)**; **SMSC endpoint NOT required** (forwards to the reverse proxy) | **forbidden** (Mode B is reverse-only) | server cert+key + trust store (client certs); routing table; **no OIDC** |
| **reverse** | **OIDC provider**; client trust store anchoring the forward proxy's internet-leg server cert (AD-26, never `cacerts`); SMSC endpoint | **OIDC provider**; opt-in ack; SMSC endpoint (legacy client connects DIRECTLY to the reverse — no forward proxy exists) | **OIDC provider**; client cert+key + trust store (internet leg); SMSC endpoint |
...
- **Rule:** *(AMENDED 2026-08-18 — the SMSC leg is always plaintext; the only Netty TLS-egress leg is the reverse→forward internet leg, where the reverse is the TLS client validating the forward's server cert, AD-12/AD-26)*: on that leg, **explicitly set `SslContextBuilder.endpointIdentificationAlgorithm`** — `null` for IP-addressed forward targets (or provision forward certs with IP SANs and keep verification on) — because Netty 4.2 defaults it to `HTTPS` and fails raw-IP connects. The reverse→IdP link is JDK `HttpClient` with standard HTTPS endpoint verification always on (AD-12). There is **no SMSC-leg TLS** — the pre-amendment "operator-choice, default off" posture is retired with the leg itself.

...
> A headless, operator-configured, open-source SMPP 3.4 security-transit proxy. The PRD (`status: final`) is the binding product contract; this spine fixes only the architecture-level invariants and resolves the items the PRD deferred here (OQ-4, OQ-11, the relay core, the module boundary, config/fail-fast, the OIDC flow, the threat model). Decisions, not rationale — rationale lives in `.memlog.md`. Versions web-verified 2026-07. Revised after the 8-lens reviewer gate (see `reviews/`).

## Design Paradigm

**Event-loop relay.** A pipes-and-filters **data plane** on Netty `ChannelPipeline`s owns the steady-state byte relay; a **control plane** of structured virtual-thread concurrency (`StructuredTaskScope` + `ScopedValue`) owns bind adjudication (OIDC), graceful shutdown, and lifecycle. Virtual threads never carry steady-state bytes; Netty event loops never run on virtual threads.

- **Data plane** (Netty event loops, platform threads): each socket leg is a pipeline `SslHandler → SmppFrameDecoder → SmppCodec → BindInterceptor → RelayIngressHandler` (ingress) / `SslHandler → SmppFrameDecoder → SmppCodec → RelayEgressHandler` (egress — the per-leg relay handlers of the Story 3.4 T5 split over the shared `CoupledRelayHandler` base). The bind→couple transition (AD-25) is a one-way flag write; post-bind the pair is coupled into a bidirectional framed-PDU relay.
- **Control plane** (virtual threads, off the event loop): `BindAdjudicator` (ROPC → OIDC provider → `Verdict`), lifecycle/graceful-drain, Spring-managed beans *(the JWKS cache died with local JWT verification, Story 3.4 T2, 2026-08-27 — AD-8 amendment)*.

```mermaid
flowchart TB
  subgraph DP["Data plane — Netty event loops (platform threads)"]
    direction LR
    IL["Ingress leg<br/>SslHandler→FrameDec→Codec→BindInterceptor→RelayIngressHandler"]
    EL["Egress leg<br/>SslHandler→FrameDec→Codec→RelayEgressHandler"]
  ...
  IL -->|"on decoded bind_*_resp ROK: RelayEgressHandler couples the pair → relay"| EL
```
...
- **Rule:** Netty event loops own the steady-state byte relay; virtual threads own the control plane (bind adjudication, OIDC, graceful shutdown). Virtual threads never carry steady-state bytes. *(Settled by user over the VT-per-connection alternative; PRD positioning line "virtual-thread relay" is reconciled — see Cross-artifact items.)* *(Wording: "steady-state byte splice" → "steady-state byte relay", Story 3.4 T4 couple/relay vocabulary unification, 2026-08-28.)*

### AD-2 — Relay coupling + framed-PDU relay
...
- **Rule:** build the control plane on **`StructuredTaskScope` (JEP 505) + `ScopedValue` (JEP 506)** — structured concurrency for the bind-adjudication fan-out (fork the ROPC token call — the only arm since the local JWKS defense-in-depth verify was removed, Story 3.4 T2, 2026-08-27 — join, collapse to one `Verdict`), graceful shutdown, and lifecycle. `ScopedValue` carries the per-bind `RequestContext` (`system_id`, `ChannelId`, deadlines) — **never `ThreadLocal`**; the scope's lifetime bounds the fan, so a denied/timed-out/cancelled adjudication tears its tasks down with the scope (no orphans). **Adoption note (reverses the prior "stable-primitives-only" stance):** `StructuredTaskScope` is **preview-only on JDK 25** (5th preview, JEP 505; 6th preview JEP 525 in JDK 26; not final until ~JDK 27) while `ScopedValue` is **final** (JEP 506). Using STS requires `--enable-preview` **process-wide**, placing the whole runtime (Netty, Nimbus, the codec) under preview semantics — an **accepted risk** (register). Pin the exact JDK 25 build + the STS preview shape so toolchain drift cannot silently change semantics; confine STS to the control plane (`security/` adjudication + `bootstrap/` lifecycle) so the data-plane relay (event loops) stays pure-stable API. **Composition with AD-28:** STS structures the fan-out *within* one adjudication; the bounded hand-managed VT entry pool (AD-28(4)) still gates how many adjudications run concurrently and supplies fail-closed-on-saturation (AD-11) — STS does **not** replace that bounding. Virtual threads carry the control plane only; Netty event loops never run on virtual threads (AD-1/AD-6).
...
- **Rule:** the ONLY mutable runtime state is (a) ephemeral per-bind connection-pair state, (b) monotonic metrics counters/gauges, (c) the JWKS cache (public keys). **Config, the routing table, and TLS material are immutable after startup validation.** `message_id → system_id` correlation never exists; **no SMS message body is ever persisted** (PRIV-1). Homes: per-bind state lives in a single concurrent **`ConnectionRegistry`** bean (keyed by ingress `ChannelId`; holds the peer-egress `Channel`, the couple flag, and ephemeral session metadata) plus a `Channel` attribute caching the entry for O(1) on the event loop; teardown removes via `channelInactive`. *(AD-32 case-3 additionally removes the entry synchronously in the violation handler and marks it tearing-down before `ctx.close()` — a case-specific override that closes the couple-race window; the later `channelInactive` removal is then a harmless no-op.)* *(AMENDED 2026-08-27, Story 3.4 T2 — clause (c) and its shape sentence RETIRED: the JWKS cache is an **`AtomicReference<JwkSet>` swapped whole on refresh** (never in-place mutation) and exposes `close()` for graceful-drain ordering (AD-22) — that cache died with local JWT verification (AD-12 amendment, same day); the mutable-runtime-state set is back to (a) per-bind state + (b) counters/gauges.)* *(Wording: "flip-flag" → "couple flag", Story 3.4 T4, 2026-08-28.)*
...
- **Rule:** the proxy is **credential-free at rest** — it holds no persisted SMPP passwords, no vault, no CA, no issuing private keys. What it *does* hold at runtime: (1) the OIDC client credential (one operational secret, file-path-injected per AD-18); (2) *(RETIRED 2026-08-27, Story 3.4 T2: cached JWKS public verification keys — died with local JWT verification, AD-12 amendment)*; (3) the per-bind plaintext SMPP password **in memory only** during adjudication and forward to the SMSC (legacy → forward proxy → reverse proxy → SMSC) — never persisted, never logged, never cached, zeroized on completion/teardown (AD-12); (4) the proxy's own TLS end-entity private key(s) — internet-leg server/client cert (AD-17/AD-29; the optional IdP mTLS client cert was REMOVED 2026-08-19 — provider client auth is `client_secret` only, Story 3.2 amendment 5) — file-path-injected (AD-18), held by the `SslContext`; not an issuing/CA key. **Blast radius:** proxy compromise yields no stealable-at-rest credential stash; a *live memory* compromise exposes only the passwords of sessions active during the compromise window — materially "network position + transient in-flight secrets," not "network position only." Trust roots are the operator's OIDC provider, the operator's PKI/trust store, and the SMSC (sole password authority). *(Brainstorm's unified-trust-root is relaxed to three roots — see Cross-artifact items.)*

### AD-11 — Fail-closed universal default  · `[ADOPTED]`
- **Binds:** FR-SEC-5.
- **Prevents:** accept-on-indeterminate anywhere in an auth path.
- **Rule:** every auth-adjacent decision denies on indeterminate. **OIDC** *(AMENDED 2026-08-27, Story 3.4 T1+T2, user-directed — the local defense-in-depth arms died: the JWT signature/iss/aud/exp/nbf checks, the 200-overridden-by-failed-local-verify precedence, the `kid`-miss/background-refresh arm, and the whole RFC 7662 introspection path; see AD-12 amendment + `.memlog.md`)*: token-endpoint 401 → DENY; any response that is not a 200 carrying a three-segment (JWS) `access_token` → DENY (3xx, 4xx≠401, 5xx, timeout, network error, 200-with-non-JWT/HTML/empty body — the JWT-only policy arm, Story 3.4 T1/D6); token content past the segment count is never examined (D7 — the endpoint verdict alone is the verdict). **Mode C:** no client cert or cert not reaching a configured anchor → TLS handshake fails (**REQUIRE never WANT**). **Ingress routing:** a `system_id` not in the routing table → DENY (no default route). **Startup:** trust store absent/empty → fail-fast refuse. Deny-on-ambiguous, always.

### AD-12 — OIDC via ROPC, disposable JWT verdict, `BindCredentialVerifier` port
- **Binds:** FR-AUTH-1, SEC-3.
- **Prevents:** verdict-cache security hole; tight coupling to a specific IdP token API (AD-36 records the rejected vendor-specific SDKs — `keycloak-authz-client`/`keycloak-admin-client` — that would re-introduce it).
- **Rule:** validate the SMPP password-grant via **ROPC** (Keycloak Direct Access Grants) — the **token-endpoint status (200/401) is the verdict** *(AMENDED 2026-08-27, Story 3.4 T1+T2, user-directed: the verdict is the token endpoint's own HTTPS-authenticated response ALONE — the RFC 7662 opaque-token introspection fallback (T1) and the local JWT signature/claim verification against cached JWKS (T2) are both removed; the proxy is the issued token's ONLY consumer — it never forwards, caches, nor shows it to anyone — so the TLS client-authenticated provider link is the sole trust anchor, and the only surviving token check is the STRUCTURAL three-segment JWS gate, the provider "issues JWTs" requirement; see .memlog)*; **discard the token and relay the ORIGINAL bind** to the SMSC (sole credential authority). **Re-validate every bind (no verdict cache).** *(A 200 body whose token is not a three-segment JWS → DENY + WARN naming the JWT-only policy and the operator remediation — the runtime fail-closed arm, Story 3.4 T1/D6; token shape is observable only at bind time.)* **Provider link (SEC-3):** the proxy→provider token endpoint is reached **over TLS (HTTPS), never HTTP** *(the JWKS endpoint died with local verification, Story 3.4 T2)*; the proxy authenticates as a confidential client via **`client_secret` only** (the RFC 8705 mTLS arm was REMOVED 2026-08-19, pre-release — Story 3.2 amendment 5), credential file-path-injected (AD-18); fail-fast at startup if the provider URL is not `https`; if OIDC discovery advertises `grant_types_supported`, probe it for Direct Access Grants (non-default since Keycloak 26.2) and warn — otherwise defer to runtime: a grant-type error from the provider → DENY per AD-11 (the DAG-enabled predicate is not reliably startup-detectable; discovery is optional). **Secret hygiene:** password (a Netty `AsciiString` per CODEC-024/PRIV-1, zeroized via its backing `array()`) and access token (`char[]`/`byte[]`) are held in zeroizable, never-`String` form, zeroized on adjudication completion, connection teardown, JVM shutdown, and any exception path. *(The `exp`/`nbf` clock precondition retired with local claim verification, Story 3.4 T2 — the endpoint verdict carries no local time dependency.)*
- **Port contract** (single interface, owns the seam): `smpp.companion.proxy.security.BindCredentialVerifier` — `VerdictRequest verify(BindCredential cred, ScopedValue<RequestContext> ctx)` (see Cancellation below); `BindCredential(SystemId, Password password)` lives in `security` (`Password` = the typed `AsciiString`-backed secret); `sealed interface Verdict permits Allow, DenyInvalid, DenyIndeterminate` (no `Throwable`, no free-form reason, no Nimbus type crosses the port — the ROPC adapter absorbs Nimbus). Runs on the relay's hand-managed VT pool (AD-6/AD-28); never `@Async`. **Cancellation (AD-32):** the port MUST expose a handle that aborts the underlying HTTP ROPC call (not only the `CompletableFuture`) — `verify` returns a **`VerdictRequest`** exposing `CompletableFuture<Verdict> future()` + `void cancelHttp()` (the ROPC adapter binds `cancelHttp()` to the HTTP client's abort; `ScopedValue<RequestContext>` carries per-bind context only, not control flow) — so AD-32 case-3 teardown can spare the IdP the abandoned ROPC (the IdP-amplification mitigation in the Accepted-Risk Register is otherwise unreachable through the seam). **Password-type revision (2026-08-09, user-directed, pre-AC8-ratification):** the password is a Netty `AsciiString` (not `char[]`), wrapped in a typed `Password` record matching the codec's CODEC-024/PRIV-1 2026-07-28 override (full type-uniformity across every C-octet field); zeroized via `AsciiString.array()`. Accepted hazard: `AsciiString.toString()` lazily caches an immortal `String` — `Password`/`BindCredential` `toString()` redact, and the standing CODEC-024 P2 (AI-5) bytecode scan must forbid `toString()` on the password across codec + port before relay logging.
- **Two-proxy flow (both directions owned)** *(AMENDED 2026-08-18, user-directed — adjudication moved to the reverse role; see .memlog)* *(FURTHER AMENDED 2026-08-21, user-ratified **[B] topology** — dial direction inverted; see .memlog)*:** the **reverse role performs OIDC verification** — it is the **sole enforcement point**, adjudicating every bind's password-grant via ROPC **before forwarding anything to the SMSC** (the reverse protects the SMSC; the SMSC remains the sole credential authority). The **forward role is a trusted-side relay**: it terminates the trusted legacy leg on its own listener (no TLS/mTLS/IdP there — the trusted network is the sole gate, AD-15), holds **no OIDC material**, and relays the original bind across the internet leg. **Under [B] the forward DIALS the reverse per SMPP session (1 : 1 : 1 — one client session ↔ one internet-leg connection ↔ one reverse-side SMSC session) and the REVERSE holds the internet-leg TLS listener** (dial direction was never load-bearing — no carrier inbound constraint; [A1] warm-lane pools and [A2] multiplexed standing connections were rejected: complexity / SMPP sequence-space demux impossibility). The forward proxy authenticates to the reverse proxy as the TLS client. **In Mode C** the reverse proxy validates the forward's **per-instance** mTLS client cert (`clientAuth(REQUIRE)`, AD-13). **In Mode A (one-way TLS) the reverse proxy CANNOT authenticate the forward proxy** — so the reverse proxy's internet-port listener **MUST be ACL-isolated to operator forward-proxy hosts** (mirroring A-3), or **Mode C must be used for the internet leg**; a Mode A deployment without ACL isolation is an **accepted risk** (register — the Mode A entry, retitled to its [B] residual: a **submission oracle**, not a harvest point — see the register's dated rewrite). The reverse proxy forwards to the SMSC **over plaintext** — the SMSC leg is a trusted network in every mode (no TLS/mTLS on that leg; the modes apply to the internet leg only). **Mode B has NO forward proxy — there is no forward×B cell:** the legacy client connects **directly to the reverse proxy** over the plaintext internet leg (accepted risk, opt-in ack); the reverse still adjudicates via ROPC before the SMSC — "plaintext + ROPC," a strengthening vs the pre-amendment model where Mode B had no OIDC anywhere.
...
- **Ratification (AC8, 2026-08-10):** the `BindCredentialVerifier` port contract above is **ratified and immutable henceforth** — validated, *before `relay/` commits against it* (Story 2.1, the Epic 2 opener), across **all four AD-12 paths** against a pinned Keycloak ≥26.7.0: (1) JWT happy path → `Allow`; (2) RFC 7662 introspection `active:true` → `Allow`; (3) RFC 8705 mTLS provider auth (**no** `client_secret`) → `Allow` — the "most likely to fail" path works, so the shape can express every path; (4) ≥1 DENY branch end-to-end (4xx → `DenyInvalid`; non-4xx / timeout / network error / JWKS `kid`-miss / claim mismatch → `DenyIndeterminate`), plus `cancelHttp()` aborting the underlying `HttpClient` exchange per AD-32. **No contract-revision finding** — the `Verdict` / `BindCredential` / `VerdictRequest` / `verify(ScopedValue<RequestContext>)` shape (incl. the 2026-08-09 `Password`-over-`AsciiString` revision) expresses all four paths unchanged; the AC3 `cancelHttp()` binding was verified against the JDK 25 source (`MinimalFuture.cancel(true)` → `MultiExchange.cancel` → `connection.close()`) and needs no revision. The full production ROPC adapter stays in Epic 3 behind this **unchanged** port; the test-tier slice that ratified it (`RopcSlice`) lives under `proxy/src/test`. *Discipline: no silent closure — a path that proved the shape unexpressive would have been recorded as a contract revision here, not silently worked around.* *(2026-08-19: path (3)'s RFC 8705 mTLS provider auth stands as the ratified historical result — the test-tier slice stays green — but is no longer a production config option; see the Provider-link rule above.)* *(2026-08-27, Story 3.4 T1+T2: paths (2) RFC 7662 introspection and (4)'s `kid`-miss DENY branch are likewise retired from the PRODUCTION adapter — JWT-only adjudication with the TLS link as sole trust anchor; the AC8 ratification itself stands as the historical record, and the test-tier slice (`RopcSlice*`) keeps all four paths green unchanged.)*
...
- **Rule:** validate peer certs to the operator-supplied trust anchor via `SslContextBuilder.trustManager(trustStore).clientAuth(REQUIRE)`; **rely on PKIX defaults for chain depth** (maxPathLength 5, tightened by `BasicConstraints.pathLenConstraint`) — do not override, no custom `PKIXBuilderParameters`, **no custom chain-validation code**. **The trust store never falls back to JDK `cacerts`** — this applies to **every peer-certificate validation path**, not only Mode C: Mode C client certs (the forward validating the reverse's client cert) AND Mode A/C validation of the forward proxy's internet-leg server cert by the reverse (AD-26; AMENDED 2026-08-18 — the SMSC leg is plaintext, so there is no SMSC-cert validation path). Use a minimal single-purpose trust store (operator issuing-CA roots only); endpoint-identification is a no-op on client certs by JSSE design. **FR-AUTH-3:** Mode C client certs are **per-instance** (one cert per runtime instance, used for all egress targets; per-target cert IDs permitted via AD-29's optional `tlsContextId`); **never a shared golden-image key**. Trust-store startup validation: refuse if absent, empty, wrong format, wrong password, or containing zero `trustedCertEntry` entries. Revocation OUT (re-deploy-to-rotate).
...
| --- | --- | --- | --- |
| **forward** *(listener = plaintext trusted leg; [B] dials the reverse per session)* | trust store validating the reverse's internet-leg server cert (AD-26, never `cacerts`); routing table; **no OIDC — trusted-side relay (AD-12 amended)**; **SMSC endpoint NOT required** (dials the routing target) | **forbidden** (Mode B is reverse-only) | forward-A material + **per-instance client cert+key** presented on every dial (FR-AUTH-3); routing table; **no OIDC** |
| **reverse** *(listener = internet-leg TLS; [B] holds it)* | **OIDC provider**; internet-leg **server cert+key** (the listener presents it; no trust store — one-way TLS validates no peers); SMSC endpoint | **OIDC provider**; opt-in ack; SMSC endpoint (legacy client connects DIRECTLY to the reverse — no forward proxy exists) | **OIDC provider**; internet-leg server cert+key + trust store REQUIRE-validating the forward's client cert (AD-13); SMSC endpoint |
...
- **Rule:** on SIGTERM, Spring lifecycle (`SmartLifecycle` / graceful-shutdown callback) drives: stop the Netty SMPP acceptor (no new binds) → **DENY in-flight bind adjudications** (fail-closed; no new adjudications after the acceptor stops) → drain in-flight relayed traffic up to a configured timeout (enumerate the `ConnectionRegistry`, AD-8) → VT control-plane drain → exit. Spring's graceful-shutdown timeout bounds the drain window. *(AMENDED 2026-08-27, Story 3.4 T2 — the two key-cache steps (stop the JWKS refresh, close the JWKS cache) died with local JWT verification and the AD-28(2) refresh executor; the adapter's stop body is deny-in-flight → release the shared client + the client secret.)*

### AD-23 — No native-image build target in v1 (resolves OQ-11)
- **Binds:** MAINT-5, OQ-11.
- **Prevents:** a second, underperforming build matrix; a ZGC regression.
- **Rule:** v1 ships the JVM build (runnable JAR + Docker) only. Keep the codebase **native-image-compatible as a documented stretch** (avoid reflection-heavy patterns in codec/relay) but produce no native binary. Revisit only if a cold-start-sensitive deployment shape (edge/serverless/memory-hard container) emerges. *(ZGC is unavailable in native-image — a native build would regress the flagship GC for an always-on single instance.)*
...
### AD-25 — Bind→couple transition state machine
- **Binds:** relay core, FR-TRANSIT-1, AD-2.
- **Prevents:** two couple units; a couple on a non-ROK `bind_resp`; divergent post-couple `unbind` handling.
- **Rule:** the couple flag is set by **exactly one unit — the data-plane `RelayEgressHandler`** (the egress leg of the Story 3.4 T5 direction-split over the shared `CoupledRelayHandler` base; the control-plane `BindAdjudicator` never mutates a data-plane flag — it returns a `Verdict` to the `BindInterceptor`, and the INGRESS leg class has no bind-response arm at all, so the couple is structural-by-TYPE). **Trigger:** on the ingress channel's event loop, upon receipt of the **decoded `bind_*_resp` with `command_status == ESME_ROK`** (codec live pre-couple) — never a peeked `command_id`, never "any frame from egress." A non-ROK `bind_resp` → do **not** couple; tear down. **`bind_*_resp` forwarder:** `BindInterceptor` forwards the `bind_*_resp` to the legacy client (it owns bind-family pre-couple per AD-3); `RelayEgressHandler` observes the decoded `bind_*_resp` **read-only** to set the couple flag and **never forwards it** — the first relayed PDU is the first PDU **after** the `bind_*_resp`. **Post-couple:** `unbind`/`unbind_resp` are opaque framed bytes forwarded like any other PDU; the session ends on TCP half/close (`channelInactive`) on either leg, which tears the per-bind pair (AD-8). (Pre-couple, `unbind` is closed per AD-32 — no cooperative handling remains.) **Close-on-violation (AD-32):** closing the connection on a pre-couple non-bind violation is an **above-spec fail-closed product policy, always on** (SMPP 3.4 prescribes the error `_resp` while remaining in state) — not SMPP-mandated, no config knob; it implicitly fails any in-flight bind without a `bind_resp` (acceptable per AD-11). **Verdict-callback re-check (race-free, with AD-32):** BOTH the Allow/couple path AND the Deny callback (`onBindReject` + `bind_*_resp`) MUST re-check the `ConnectionRegistry` entry before proceeding — if the entry is absent or marked tearing-down (AD-32 case 3 tore it down on a concurrent pre-couple violation, and its `cancelHttp()` makes the aborted-verdict land near-instantly), the callback is a no-op (no couple, no `onBindReject`, no `bind_resp`). This upholds AD-32 case 3's invariant that Q1 never emits a `bind_*_resp` (or any `_resp`) on a connection it is tearing down. *(Defense-in-depth: case-3 teardown also shuts the adjudication's `StructuredTaskScope` per AD-5, so a late Verdict is unlikely to be produced at all.)* *(AMENDED 2026-08-28, Story 3.4 T4 — title re-termed "Bind→splice transition state machine" → "Bind→couple transition state machine" and the rule's splice/flip wording swept to the couple vocabulary ("the splice flag is flipped" → "the couple flag is set", "flip" → "couple", "spliced PDU" → "relayed PDU"); the mechanism is unchanged — single-flipper became single couple unit, still the data-plane `RelayHandler` on the decoded ROK.)* *(AMENDED 2026-08-28, Story 3.4 T5 — the single couple unit `RelayHandler` was SPLIT (D3, owner checkpoint same date) into the per-leg `RelayIngressHandler`/`RelayEgressHandler` over the abstract base `CoupledRelayHandler`; the rule text now names `RelayEgressHandler` (both sites above), and the single-unit requirement became STRUCTURAL-BY-TYPE: only the egress class can express the couple (`ConnectionEntry.couple()` is called from exactly one main class — pinned by an ArchUnit call-site rule, AC6); the ingress class has no bind-response arm — a stray decoded `bind_resp` there fails closed (bare-close, never couples). The mechanism is unchanged: still exactly one unit, on the decoded ROK, on the ingress channel's event loop (the egress leg rides it, AD-2). Same marker sweeps the type's other spine naming sites: the Design-Paradigm data-plane bullet, the mermaid leg labels + couple edge, AD-7's couple-trigger sentence, AD-27's "holds an injected reference" clause (re-pointed to the per-leg handlers over the base), and the module map.)*

### AD-26 — Egress TLS trust anchoring
- **Binds:** SEC-1, SEC-3, AD-13, AD-20.
- **Prevents:** trust-root collapse on the egress side (any publicly-trusted cert authenticating a MitM internet-leg peer).
- **Rule:** the forward proxy's TLS client validates the peer (the reverse proxy's internet-leg server cert; the SMSC leg is plaintext per the amended AD-12 two-proxy flow — and the dial is forward→reverse under the [B] amendment of 2026-08-21) against an **operator-supplied trust store with the same posture as AD-13 — never JDK `cacerts`**, fail-fast if absent/empty. Disabling hostname verification (AD-20) is conditional on either (a) IP-SAN-provisioned peer certs with verification ON, or (b) the operator internet-leg trust store being the sole gate. If an operator explicitly requires `cacerts`/public-PKI trust for the internet-leg peer, it is an **explicit opt-in with a loud startup warning + acknowledgment, permitted in ANY mode** (the threat — trusting a public root for the internet-leg peer — is identical across modes; this is the Mode B pattern, not Mode-B-only) — never the default. *(AMENDED 2026-08-18: the egress TLS peer is the forward's internet-leg server cert — the pre-amendment "SMSC peer / SMSC trust store" wording is retired with the plaintext SMSC leg.)*

### AD-27 — Intra-`proxy` ownership seams
- **Binds:** AD-3, AD-7, AD-19.
- **Prevents:** two owners of the bind-family set or the relay metric; double/under-counting.
- **Rule:** (1) the **bind-family `command_id` set** is owned by `codec` as the single source of truth (`SmppCommandIds.BIND_FAMILY`); `BindInterceptor` (`relay/`) MUST consume it — redefining it locally is forbidden. `outbind` and `generic_nack` are declared **opaque-relayed, not bind-family**. (2) A **`RelayObserver`** interface is owned by `observability/` (methods: `onFramedPdu(Direction)`, `onBindAccept(SystemId)`, `onBindReject(SystemId, Verdict)`, `onConnectionClosed(Direction, CloseReason)` — no PDU type, no content; `CloseReason` is a closed enum exhaustive over the spine's close paths: `{PEER_HALF_CLOSE, PEER_RST, EGRESS_CONNECT_FAILED, OVERSIZED_FRAME, UNDERSIZED_FRAME, DECODE_ERROR, UNKNOWN_COMMAND_ID, PRE_COUPLE_NON_BIND_PDU, CLEAN_UNBIND_HANDSHAKE, GENERIC_NACK_PRE_BIND, BIND_REJECTED, BIND_FAILED_NON_ROK, INGRESS_TLS_HANDSHAKE_FAILED, EGRESS_TLS_HANDSHAKE_FAILED, SHUTDOWN_DRAIN, OTHER}`). **Trigger events are pinned:** `onBindAccept` fires exactly at the AD-25 couple (decoded `bind_*_resp` ROK), not at the verdict; `onBindReject` is called **only** for `Verdict` values returned by `BindCredentialVerifier` (`DenyInvalid`/`DenyIndeterminate`) — routing-miss, `kid`-miss, and config denies go to AD-19's unlabeled counter, never through `onBindReject`. The `RelayObserver` implementation **MUST apply AD-19's cardinality rule** (a `SystemId` not in the routing table is never a label). The per-leg relay handlers (`RelayIngressHandler`/`RelayEgressHandler` over the `CoupledRelayHandler` base — the Story 3.4 T5 split, 2026-08-28) hold an injected reference and call it; **the codec never emits metrics**. One counter source; the Micrometer impl is the only thing behind the seam. **Connection-close telemetry (AD-32):** `onConnectionClosed` is **exactly-once per channel** (idempotent, CAS-guarded on the channel attribute; owned by the `channelInactive` teardown site — the violation handler only stashes the `CloseReason`, never calls `onConnectionClosed` directly) and **fires for every `channelInactive`**. It backs `relay_connections_closed_total{direction, reason}` (cardinality = 2 × |CloseReason|, bounded; spans both pre- and post-couple windows; NO `system_id`/`command_id`/`ChannelId` label ever — AD-19). This replaces the latent RELAY-010 "increment-on-teardown" assumption; the full 4-method shape (incl. `onConnectionClosed`) is seeded by Epic 2's noop impl, so Epic 4 swaps only the impl behind the unchanged interface. *(AMENDED 2026-08-28, Story 3.4 T4 — `SpliceObserver` RENAMED `RelayObserver` (type rename behind an otherwise-unchanged seam, the memlog L105 5→4-method precedent; the 4 method names unchanged); "the splice metric" → "the relay metric", "AD-25 flip" → "AD-25 couple", "opaque-spliced" → "opaque-relayed". No trigger, cardinality, or ownership change.)*
...
- **Rule:** (1) the `SslHandler` delegating-task `Executor` is **ONE hand-managed fixed platform-thread pool** (NOT virtual threads — SSLEngine delegated tasks are CPU-bound and may pin carriers; NOT a Spring `ThreadPoolTaskExecutor`), shared by ingress + egress, bounded queue, abort-and-fail-handshake on saturation (AD-4). (2) *(AMENDED 2026-08-27, Story 3.4 T2 — arm RETIRED: JWKS refresh was a **hand-rolled virtual thread on a `ScheduledExecutorService` owned by `security/`** (not `@Scheduled`), stopped by the AD-22 drain **before** the JWKS cache closes; that executor died with the cache and local JWT verification (AD-12 amendment, same day) — Story 3.4 T2 REMOVES an executor and adds none, so arm (4)'s adjudication pool is the security plane's sole own-executor.)* (3) `spring.threads.virtual.enabled` (AD-6) permitted only for Spring's own internal executors. (4) **Bind adjudication runs on a single bounded hand-managed VT `ExecutorService` (JEP 444) owned by `security/`**, shared across all binds, sized via config, fail-closed on saturation (AD-11), registered with the AD-22 drain (DENY-in-flight = `shutdownNow()` + await); **spawning one VT per bind outside this pool is forbidden** — this is the executor AD-12 references.
...
- **Rule:** a **v1 forward-role instance fronts exactly one carrier egress (1:1)**; the routing table is a `system_id` allow-list, every permitted `system_id` mapping to the **single** configured egress target. Multi-carrier routing (`system_id` → per-carrier egress) is **deferred to post-v1** (simplicity + single-instance + statelessness + bounded metrics cardinality). Value schema: `{host, port, tlsContextId?}`; `tlsContextId` optional — if absent, the instance-level TLS context is used. Mode C client cert is **per-instance by default**; per-target cert IDs permitted via `tlsContextId` + an explicit `forward.tls-contexts` map. *(User-confirmed 1:1 for v1; multi-carrier remains deferred — see Deferred.)* *(AMENDED 2026-08-26, Story 3.3 checkpoint review: the override map moved from `companion.tls.contexts` to `companion.forward.tls-contexts` — forward-scoped because only the forward dials TLS under [B]; the top-level `companion.tls` node keeps policy only.)*
...
- **Rule:** `SmppFrameDecoder` enforces a single pinned **max `command_length` (65 536 octets — covers `message_payload` TLV max)**; frames exceeding are dropped and the channel closed (fail-closed). The decoder rejects declared lengths < 16 and guards length-arithmetic overflow/underflow **before allocation** (independent of `-XX:MaxDirectMemorySize`). Per-channel inbound queue depth (in framed-PDU buffers) is bounded; `AUTO_READ=false` + write-completes-gates-read (AD-2) is the trip-wire with an explicit low-water mark to re-arm read. `MaxDirectMemorySize` is derived from `max_frame × max_inbound_depth × concurrent_pairs × safety_factor` (in `MemoryBudget.compute`, referencing the codec's `MAX_COMMAND_LENGTH` so the codec max and the allocator budget cannot drift); the allocator's `ByteBufAllocatorMetric` is surfaced to operators via `/metrics` (Epic 4). **Shared-constant assertion (resolves test-design Q5):** the codec max `command_length` (65 536), the `MaxDirectMemorySize` formula input, and the config default MUST reference ONE named constant — asserted by an ArchUnit/Gradle static test (catches conservative drift a runtime soak cannot). PLUS a startup self-check: live `MaxDirectMemorySize` MUST be ≥ the computed budget, else fail-fast (AD-17) by default — closes the JVM-flag gap the ArchUnit test cannot reach. The self-check runs for EVERY role×mode cell (every deployment has exactly one cell, AD-17 — the budget is a property of the relay data plane, and every cell relays). The over-budget severity follows the `companion.memory.budget-check` policy: `fail` (the default, including an absent key) refuses startup; `warn` — an explicit operator accepted-risk opt-in (the Mode B pattern, AD-17) — emits a loud startup banner and starts. No value skips the check itself: it always computes and compares. The live ceiling is read WITHOUT JDK-internal API: the explicit `-XX:MaxDirectMemorySize` if set, else `Runtime.maxMemory()` (the `-Xmx` default); `ByteBufAllocatorMetric` exposes only current *usage*, not the ceiling. *(Amended 2026-08-12, Story 2.2 T5 review: implemented via `ManagementFactory`/`Runtime.maxMemory()` — not `jdk.internal.misc.VM` (would need a production-jar `--add-exports` that breaks `java -jar`) and not `ByteBufAllocatorMetric`.)* *(Amended 2026-08-15, Story 2.2 T5b, operator decision: the self-check is UNCONDITIONAL across all role×mode cells (T5 had scoped it to `reverse.mode-b`, the slice that story wired — Epic 3 no longer needs to widen the guard) + the `companion.memory.budget-check: fail | warn` over-ceiling policy (default `fail`, i.e. the prior behavior; `warn` = loud banner + start, an explicit accepted-risk opt-in).*
...
- **Rule:** on EITHER leg, BEFORE the AD-25 couple (decoded `bind_*_resp` ROK), a received PDU is handled by case — fail-closed, **zero config knobs**, uniform above-spec close:
  - **bind family** (`bind_*` / `bind_*_resp`) → handled per AD-3/AD-25 (the bind handshake and the ROK couple). These are the **only** pre-couple PDUs that receive a cooperative response.
  - **egress `generic_nack` (or non-ROK `bind_*_resp`) from the SMSC, pre-`bind_resp`** → **forwarded verbatim** to the legacy ESME as the bind result — the SMSC is the sole credential authority, so its actual response is ground truth and passes through unchanged (no couple; tear down both legs). (Distinct from the proxy's OWN pre-SMSC denials — IdP/verifier or routing-miss — which collapse per AD-33; and from SMSC transport-death — connect-fail/RST/`unbind`/half-close with no SMSC response PDU — which the proxy synthesizes via AD-33.)
...
- **Rule:** ship a conservative default, operator-tunable under `companion.tls.*`. **Protocols:** `["TLSv1.3", "TLSv1.2"]` (1.3 preferred). **TLS 1.2 cipher set:** `TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384`, `TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256`, `TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384`, `TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256` (no CBC, no static-RSA key exchange, no RC4/3DES/null/anon); `TLS_ECDHE_*_WITH_CHACHA20_POLY1305_SHA256` optional where the provider offers it. At startup, intersect the configured set with each `SSLContext`'s supported suites **for EVERY context** (ingress server + every egress target context + the IdP client context, AD-12/AD-29); **each empty intersection → fail-fast** (AD-17). **TLS 1.3 ciphers:** restricted to the JDK AEAD set `{TLS_AES_256_GCM_SHA384, TLS_AES_128_GCM_SHA256, TLS_CHACHA20_POLY1305_SHA256}` (parity with the TLS-1.2 posture; ChaCha20 optional). *(Resolves the Deferred "exact cipher allowlist" item — the default is pinned; operators may tune.)*
...
### AD-36 — No vendor-specific OIDC SDK behind the `BindCredentialVerifier` port (`keycloak-authz-client`/`keycloak-admin-client` rejected)  · `[ADOPTED]`
- **Binds:** AD-12, AD-16, SEC-4.
- **Prevents:** re-introduction of the IdP-token-API coupling the AD-12 port exists to localize; a second HTTP/crypto stack on the runtime classpath; erosion of the `cancelHttp()`/fail-closed/zeroization contract.
- **Rule:** the production ROPC adapter (and any test-tier slice behind the same port) speaks **plain OIDC** over the JDK builtin `java.net.http.HttpClient` *(AMENDED 2026-08-27, Story 3.4 T1+T2: the adapter no longer verifies JWTs locally — Nimbus JOSE+JWT survives ONLY as the discovery-JSON parser (`JSONObjectUtils`; the proxy carries no other JSON lib), the structural three-segment gate is library-free — see AD-12/D7; the AC7 "Nimbus only" crypto-allowlist entry stands for that surviving parse use)* — **no vendor-specific Keycloak client library is admitted on any classpath** (`org.keycloak:keycloak-authz-client`, `org.keycloak:keycloak-admin-client`). Reasons: (1) the port's purpose (AD-12 "Prevents: tight coupling to a specific IdP token API") is to keep Keycloak a swappable reference IdP; a Keycloak SDK couples the adapter to Keycloak-proprietary surfaces (`keycloak.json`, RPT/UMA/PAT, Admin REST) at the very seam whose job is rework-localization — and ROPC is the #1 fragile dependency. (2) Wrong tool shape: `keycloak-authz-client` is built for Keycloak Authorization Services (`authorization(u,p).authorize()` returns a Requesting Party Token; introspection is `introspectRequestingPartyToken`) — not plain `grant_type=password` → access token + RFC 7662 `active`; it would force Authorization-Services config on the client for nothing. `keycloak-admin-client` is an admin/provisioning REST client, not a runtime OIDC token/validation client. (3) Dependency mechanics: `keycloak-admin-client` drags JAX-RS + RESTEasy/Jersey and `spring-boot-starter-oauth2-*` drags `spring-web` — both **banned by the AD-16/OBS-013 runtime-purity gate** (`smpp.runtime-purity.gradle.kts`, wired into `check`); `keycloak-authz-client` (Apache HttpClient + Jackson + keycloak-core) is a second HTTP stack against the Story-2.1 dev-note "Do not pull a third-party HTTP client." (4) Contract fidelity: the adapter runs on the AD-28(4) hand-managed bounded VT pool, fail-closed on saturation, and `VerdictRequest.cancelHttp()` (AD-32 case-3) must abort the underlying wire call; blocking Keycloak clients expose no per-request cancel handle, so raw `HttpClient` would be retained for the ROPC call anyway — no net win — while also obstructing the AC5 zeroization of the `Password`/token bytes. (5) AC7 `NoRolledCryptoArchitectureTest` enforces "JDK SSLEngine/SSLContext + Nimbus only"; admitting a Keycloak SDK would require both an AD-12 revision and a test exemption. The subsumed surface is ~120 lines of small, isolated, fully-tested (all four AD-12 paths against Keycloak 26.7.0) OIDC code — the cost/benefit is adverse. The production adapter (Epic 3) stays on this same HttpClient+Nimbus stack the test slice ratifies.
- **Narrow test-only carve-out (not now):** `keycloak-admin-client` on the **test** classpath would be the right tool *only if* a future story requires imperative, per-test realm/client/user provisioning. Today's deterministic realm-import JSON (`realm-smpp-companions.json`) is simpler and sufficient; the dedicated `org.testcontainers:keycloak` module was already evaluated and rejected (cannot do `KC_HTTPS_CLIENT_AUTH=required` mTLS readiness). Revisit only when a concrete dynamic-provisioning need appears — and weigh the heavy JAX-RS/RESTEasy transitive pull against the simpler JSON-import status quo.
- **Honesty:** this does not add security; it records the rejected alternative so it is not re-litigated, and elevates the existing Story-2.1 dev-note ("Do not pull a third-party HTTP client") to a formal register decision.
...
- **Mode A two-proxy without ACL isolation** — *(REWRITTEN 2026-08-21 for the [B] topology — the previous "password harvest point" framing RETIRES with the dial-direction flip; see .memlog)*: in one-way TLS the reverse proxy cannot authenticate the forward (the forward is the unauthenticated TLS client), so any peer reaching the reverse proxy's internet listener may SUBMIT binds. The harvest risk is gone — the reverse adjudicates every submitted credential via ROPC and the SMSC remains the sole credential authority, so a fake "forward" harvests nothing; the residual is a **submission oracle**: an attacker can drive credential GUESSES through the reverse's ROPC screen and SMSC (no worse than the Mode B posture, same no-rate-limit family). Mitigation = ACL-isolate the reverse proxy's internet listener to operator forward-proxy hosts (mirroring A-3) or use Mode C (AD-12).
...
- Authority-provider outage blocks new binds — ongoing relayed sessions survive (adjudication is bind-time only; no provider state is cached) *(re-worded 2026-08-27, Story 3.4 T2 — the "survive on cached JWKS" clause died with the cache)* *(wording: "ongoing splices" → "ongoing relayed sessions", Story 3.4 T4, 2026-08-28)*. (A-2).
...
- **ROPC hard-dependency — *new from research.*** v1 architecture **hard-depends on ROPC (Direct Access Grants)**, a deprecated grant (RFC 9700 "MUST NOT"; OAuth 2.1 removes). The first-party/headless/in-memory framing places this build outside the deprecation's *primary* rationale (3rd-party end-user apps), but does **not** neutralize the normative language or the trajectory. The `BindCredentialVerifier` port (AD-12) **localizes a future rework to one adapter; it does not eliminate the dependency** — no standard replacement grant exists. Keycloak 26.7 still ships Direct Access Grants; if it is removed, v1 must be reworked. This is the single most fragile external dependency in the trust model.
  - *Story 3.1 viability verdict (2026-08-08):* **MET — ROPC-conditional path not foreclosed; no fallback DECISION is invoked.** Primary-confirmed: Keycloak 26.7.0 (GA 09 Jul 2026; docs 26.7.1) ships DAG (grant_type=password), not deprecated, and per Keycloak release policy cannot be removed before KC 27. No fallback is invoked — the pre-enumerated tree triggers only on probe *failure* or a removal notice (epics.md §Epic 3); neither occurred. **Pin sharpened to ≥26.7.0, re-validated each minor** (no upstream LTS; minors EOL ~3 months). **Removal-watch live:** RFC 9700 §2.4 final BCP "MUST NOT"; OAuth 2.1 draft-15 omits ROPC; Keycloak docs "MUST NOT be used"; maintainer enforcement roadmap — a 26.x minor could tighten ROPC *without* a formal removal notice. **Scope residue:** viability validates AD-12 path (1) JWT happy path only; paths (2) RFC 7662 introspection, (3) RFC 8705 mTLS provider auth (no primary ROPC+mTLS example — most likely to fail), (4) ≥1 DENY branch remain for Epic 2's opener + Epic 3's full adapter. **Update (2026-08-10): Epic 2's opener (Story 2.1) validated all four paths against Keycloak 26.7.0 — see AD-12 Ratification (AC8); path (3) mTLS works (no `client_secret`); the port is immutable henceforth and Epic 3 ships the production adapter behind the unchanged port.** **Update (2026-08-19): the RFC 8705 mTLS provider-auth option was removed from the production config surface (user-directed, Story 3.2 amendment 5) — provider client auth is `client_secret` only; path (3) survives in the test-tier ratification slice (`RopcSlice*`) and Epic 3's production live suite is 3-path.** **Update (2026-08-27, Story 3.4 T1+T2):** paths (2) RFC 7662 introspection and (4)'s `kid`-miss DENY branch are likewise retired from the PRODUCTION adapter (user-directed) — JWT-only adjudication with the TLS provider link as sole trust anchor; the AC8 ratification stands as the historical record and the test-tier slice (`RopcSlice*`) keeps all four paths green unchanged. **Unverified signal (not asserted):** CVE-2026-9792 (alleged unauth bypass of `reject-ropc-grant`, fix 26.7.0/26.6.3/26.4.13, GitHub #49436) — referenced by AI summaries, not primary-confirmed; the ≥26.7.0 pin stands independently.
- **Preview-API dependency (StructuredTaskScope) — *new.*** The control plane adopts `StructuredTaskScope` (JEP 505), which is **preview-only on JDK 25** (still preview in JDK 26 per JEP 525; not final until ~JDK 27) — `ScopedValue` (JEP 506) is already final. Using STS requires `--enable-preview` **process-wide**, placing Netty, Nimbus, and the codec under preview semantics, and the STS API shape (the "joiner" design) may change before finalization. Mitigations: pin the exact JDK 25 build; confine STS to the control plane so the data-plane relay stays pure-stable API; keep usage behind the `security/`/`bootstrap/` seam so a future API change is localized. Revisit at the next LTS where STS is final. *(Reverses the earlier "stable-primitives-only" stance — see `.memlog.md`.)*
...
| Secrets | Password (typed `Password` / Netty `AsciiString`, CODEC-024/PRIV-1) / tokens in `char[]`/`byte[]`, zeroized (AD-12); file-path-injected at rest (AD-18). |
| State & mutation | Only AD-8's mutable buckets (two since the JWKS cache died with local JWT verification, Story 3.4 T2, 2026-08-27 — AD-8 amendment); everything else immutable post-startup. Monotonic counters only. |
...
graph TB
  ...
  JAR --- MOUNT
  DOCKER --- MOUNT
  JAR -. "ROPC over TLS (the JWKS leg died with local JWT verification, Story 3.4 T2, 2026-08-27)" .-> IDP
  DOCKER -. "ROPC over TLS" .-> IDP
  JAR --> MET
```
...
companions/
  ...
  proxy/              # the runnable proxy application (the JAR + Docker image)
    src/main/java/smpp/companion/proxy/
      relay/                     # Netty pipelines, BindInterceptor, RelayIngressHandler/RelayEgressHandler over CoupledRelayHandler (split from RelayHandler, Story 3.4 T5, 2026-08-28), coupling, ConnectionRegistry
      security/                  # TLS modes A/B/C, trust-store loading, OIDC discovery-JSON parsing (Nimbus; local JWT verify removed, Story 3.4 T2, 2026-08-27), BindCredentialVerifier port + ROPC adapter
      config/                    # @ConfigurationProperties model, fail-fast validation, role×mode matrix, routing table
      observability/             # JSON-lines logging, RelayObserver (renamed from SpliceObserver, Story 3.4 T4, 2026-08-28), /metrics scrape handler (Micrometer), MetricsRegistry
...
- **Rate-limit values, ROPC timeouts** — tune to the PERF-3 latency budget; config, not invariant. *(AMENDED 2026-08-27, Story 3.4 T2: the JWKS cache TTL / refresh-ahead entries and the fixed `kid`-miss policy died with local JWT verification and the cache — no key-set tuning surface remains.)*
...
- **DLR relay mechanics** — determined by AD-2/AD-9/AD-25; per-PDU detail belongs in `relay` tests. *(Re-termed "DLR-splice byte mechanics", Story 3.4 T4, 2026-08-28.)*

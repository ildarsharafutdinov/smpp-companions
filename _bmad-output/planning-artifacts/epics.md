...
- **[AD-17]** Required-config matrix (required/optional/forbidden per cell; AMENDED 2026-08-18 — OIDC moved to the reverse role): forward+A = server cert+key (internet leg), routing table, NO OIDC (trusted-side relay), SMSC endpoint NOT required (forwards to reverse proxy); forward+B = FORBIDDEN (no forward proxy exists in Mode B — legacy client connects directly to the reverse); forward+C = server cert+key + trust store, routing table, NO OIDC. reverse+A = OIDC provider, client trust store anchoring the forward proxy's internet-leg server cert (AD-26, never cacerts), SMSC endpoint; reverse+B = OIDC provider, opt-in ack, SMSC endpoint (direct client→reverse, plaintext internet leg); reverse+C = OIDC provider, client cert+key + trust store (internet leg), SMSC endpoint.
...
- **[AD-12]** Two-proxy flow (both directions owned; AMENDED 2026-08-18 — adjudication at the REVERSE): REVERSE performs OIDC verification — sole enforcement point, ROPC before anything reaches the SMSC; FORWARD is a trusted-side relay (no TLS/mTLS/IdP on the legacy leg, no OIDC material). Reverse authenticates to forward as TLS client. Mode C → forward validates reverse's mTLS client cert. Mode A (one-way TLS) → forward CANNOT authenticate reverse, so forward's internet-port listener MUST be ACL-isolated to reverse-proxy hosts (mirroring A-3) OR Mode C used. Mode A two-proxy without ACL isolation = ACCEPTED RISK (register + loud startup warning, the Mode B pattern; a fake "reverse" harvests UNVERIFIED credentials). Reverse→SMSC is PLAINTEXT in every mode (trusted SMSC-leg network). Mode B = NO forward proxy: legacy client connects DIRECTLY to the reverse over the plaintext internet leg (opt-in ack); the reverse still adjudicates ("plaintext + ROPC").
...
- **[AD-20]** Egress TLS endpoint-identification (AMENDED 2026-08-18 — SMSC leg always plaintext; the Netty TLS-egress leg is reverse→forward): on that leg, explicitly set `SslContextBuilder.endpointIdentificationAlgorithm` — null for IP-addressed forward targets (Netty 4.2 defaults to HTTPS and fails raw-IP) OR provision forward certs with IP SANs and keep verification on. No SMSC-leg TLS exists. reverse→IdP uses JDK HttpClient with standard HTTPS verification always on.
- **[AD-26]** Egress TLS trust anchoring: reverse proxy TLS client validates peer (the forward's internet-leg server cert; AMENDED 2026-08-18 — SMSC leg plaintext, no SMSC-cert path) against operator-supplied trust store — same posture as AD-13 (never cacerts, fail-fast if absent/empty). Disabling hostname verification (AD-20) conditional on (a) IP-SAN certs with verification ON, or (b) operator internet-leg trust store as sole gate.
...
| AD-20 | Egress TLS endpoint-identification | Set endpointIdentificationAlgorithm on the reverse→forward internet leg (null for raw-IP forward targets); SMSC leg always plaintext (AD-12 amended 2026-08-18); IdP link = JDK HttpClient, verification always on. |

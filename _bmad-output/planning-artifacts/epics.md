...
- **[AD-17]** Required-config matrix (required/optional/forbidden per cell; AMENDED 2026-08-18 — OIDC moved to the reverse role; AMENDED 2026-08-21 — **[B] re-shape**, forward holds no server cert / reverse holds the listener): forward+A = trust store validating the reverse's internet-leg server cert (AD-26, never cacerts), routing table, NO OIDC (trusted-side relay), SMSC endpoint NOT required (dials the routing target); forward+B = FORBIDDEN (no forward proxy exists in Mode B — legacy client connects directly to the reverse); forward+C = forward-A material + per-instance client cert+key presented on every dial, routing table, NO OIDC. reverse+A = OIDC provider, internet-leg server cert+key (listener presents it; no trust store — one-way TLS validates no peers), SMSC endpoint; reverse+B = OIDC provider, opt-in ack, SMSC endpoint (direct client→reverse, plaintext internet leg); reverse+C = OIDC provider, internet-leg server cert+key + trust store REQUIRE-validating the forward's client cert, SMSC endpoint.
- **[AD-29]** Routing cardinality v1 = **1:1**: a forward instance fronts EXACTLY ONE carrier egress; routing table = `system_id` allow-list mapping to the single egress. Multi-carrier deferred post-v1. Value schema `{host, port, tlsContextId?}`; Mode C client cert per-instance by default, per-target via `tlsContextId` + `forward.tls-contexts` map. *(AMENDED 2026-08-26, Story 3.3 review: the map re-scoped `companion.tls.contexts` → `companion.forward.tls-contexts`; `companion.bind.max-connections` retired — the F13 cap reads `companion.memory.concurrent-pairs`.)*
...
- **[AD-12]** Two-proxy flow (both directions owned; AMENDED 2026-08-18 — adjudication at the REVERSE; AMENDED 2026-08-21 — **[B] topology**): REVERSE performs OIDC verification — sole enforcement point, ROPC before anything reaches the SMSC; FORWARD is a trusted-side relay (no TLS/mTLS/IdP on the legacy leg, no OIDC material). **[B]: the forward DIALS the reverse per SMPP session (1:1:1) and the REVERSE holds the internet-leg TLS listener.** Forward authenticates to reverse as TLS client. Mode C → reverse validates forward's per-instance mTLS client cert (REQUIRE). Mode A (one-way TLS) → reverse CANNOT authenticate forward, so the reverse's internet listener MUST be ACL-isolated to forward-proxy hosts (mirroring A-3) OR Mode C used. Mode A without ACL isolation = ACCEPTED RISK (register — retitled 2026-08-21 to its [B] residual: a submission oracle, not a harvest point). Reverse→SMSC is PLAINTEXT in every mode (trusted SMSC-leg network). Mode B = NO forward proxy: legacy client connects DIRECTLY to the reverse over the plaintext internet leg (opt-in ack); the reverse still adjudicates ("plaintext + ROPC").
...
- **[AD-13]** Trust store NEVER falls back to JDK `cacerts` — applies to EVERY peer path (Mode C client certs AND Mode A validation of the forward's internet-leg server cert, AD-26 — the SMSC leg is plaintext, AD-12 amended 2026-08-18). Mode C client certs PER-INSTANCE; never shared golden-image key. Startup validation: refuse if absent/empty/wrong format/wrong password/zero `trustedCertEntry`. Revocation OUT.
...
- **[AD-20]** Egress TLS endpoint-identification (AMENDED 2026-08-18 — SMSC leg always plaintext; AMENDED 2026-08-21 [B] — the Netty TLS-egress leg is forward→reverse): on that leg, explicitly set `SslContextBuilder.endpointIdentificationAlgorithm` — null for IP-addressed reverse targets (Netty 4.2 defaults to HTTPS and fails raw-IP) OR provision reverse certs with IP SANs and keep verification on. No SMSC-leg TLS exists. reverse→IdP uses JDK HttpClient with standard HTTPS verification always on.
- **[AD-26]** Egress TLS trust anchoring: forward proxy TLS client validates peer (the reverse's internet-leg server cert; AMENDED 2026-08-18 — SMSC leg plaintext, no SMSC-cert path; AMENDED 2026-08-21 [B] — the dial is forward→reverse) against operator-supplied trust store — same posture as AD-13 (never cacerts, fail-fast if absent/empty). Disabling hostname verification (AD-20) conditional on (a) IP-SAN certs with verification ON, or (b) operator internet-leg trust store as sole gate.
- **[AD-26]** Explicit opt-in (loud startup warning + ack) required for cacerts / public-PKI internet-leg trust; permitted in ANY mode (threat identical across modes — the Mode B pattern), NEVER the default.

**Integration**

- **[AD-12]** OIDC via ROPC (Keycloak Direct Access Grants): token-endpoint status (200/401) IS the verdict; DISCARD the token and relay the ORIGINAL bind to the SMSC (sole credential authority). Re-validate EVERY bind (NO verdict cache). *(AMENDED 2026-08-27, Story 3.4 T1+T2, user-directed: local JWT verification via cached JWKS, the introspection fallback, and the "cache JWKS only" clause are REMOVED — the token endpoint's HTTPS-authenticated response alone is the verdict, gated only by the structural three-segment JWT check; spine AD-12 carries the full amendment + .memlog entry.)*
- **[AD-12]** Provider link (SEC-3): token endpoint reached OVER HTTPS, never HTTP (the JWKS endpoint died with local JWT verification, Story 3.4 T2, 2026-08-27); proxy authenticates as confidential client via client_secret ONLY (the RFC 8705 mTLS arm was REMOVED 2026-08-19, Story 3.2 amendment 5), credential file-path-injected (AD-18). Fail-fast if provider URL not https. If discovery advertises `grant_types_supported`, probe for Direct Access Grants (non-default since Keycloak 26.2) and WARN; grant-type error at runtime → DENY (AD-11).
...
| AD-25 | Bind→couple transition state machine | Couple flag set by exactly RelayEgressHandler (split from RelayHandler with the ingress class, Story 3.4 T5, 2026-08-28 — structural-by-type) on decoded bind_*_resp ROK; BindInterceptor forwards bind_resp; RelayEgressHandler read-only; post-couple unbind opaque; session ends on TCP half/close. |
...
- **ROPC hard-dependency** — v1 hard-depends on ROPC (Direct Access Grants), a deprecated grant (RFC 9700 "MUST NOT"; removed in OAuth 2.1). The `BindCredentialVerifier` port LOCALIZES a future rework to one adapter but does NOT eliminate the dependency (no standard replacement grant). Keycloak 26.7 still ships it; if removed, v1 must be reworked. **Single most fragile external dependency in the trust model.**
  - *Story 3.1 (2026-08-08):* viability MET — Keycloak 26.7.0 ships DAG, no removal/deprecation in 26.x, **no fallback invoked**. Pin ≥26.7.0 (no LTS; re-validate each minor). DAG off-by-default → AI-3 fixture must enable it. 4-path validation (opaque/7662, mTLS/8705, DENY) → Epic 2 opener + Epic 3 (the 7662 and kid-miss legs were later retired from the production adapter, Story 3.4 T1+T2, 2026-08-27 — the 4-path record stands as the AC8 ratification). Removal-watch live.
